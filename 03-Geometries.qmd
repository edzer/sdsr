...
Geometries on the sphere are discussed in @sec-spherical,
...
```{r fig-sfgeometries,fig.cap = "sketches of the main simple feature geometry types", echo=!knitr::is_latex_output()}
#| code-fold: true
library(sf)
par(mfrow = c(2,4))
par(mar = c(1,1,1.2,1))
...
and s2geometry libraries (@fig-gdal-fig-nodetails) .
...
operations entirely focusing on _geometrical_ properties.  
...
\index{st\_is\_simple}
\index{st\_is\_valid}
\index{st\_is\_empty}
\index{st\_is\_longlat}
\index{st\_is}
...
of the _first_ geometry (the polygon): @fig-de9im  is
summarised row-wise.
...
\index{st\_contains}
\index{st\_contains\_properly}
\index{st\_covers}
\index{st\_covered\_by}
\index{st\_crosses}
\index{st\_disjoint}
\index{st\_equals}
\index{st\_equals\_exact}
\index{st\_intersects}
\index{st\_is\_within\_distance}
\index{st\_within}
\index{st\_touches}
\index{st\_overlaps}
\index{st\_relate}
...
geometries explained in @sec-de9im.
...
```{r fig-vor, out.width='60%', fig.cap="for a set of points, left: convex hull (red); middle: Voronoi polygons; right: Delauney triangulation", echo=!knitr::is_latex_output()}
#| code-fold: true
...
\index{st\_buffer}
\index{st\_jitter}
\index{st\_wrap\_dateline}
\index{st\_boundary}
\index{st\_convex\_hull}
\index{st\_line\_merge}
\index{st\_make\_valid}
\index{st\_node}
\index{st\_point\_on\_surface}
\index{st\_polygonize}
\index{st\_segmentize}
\index{st\_simplify}
\index{st\_split}
\index{st\_transform}
\index{st\_triangulate}
\index{st\_voronoi}
\index{st\_zm}
\index{st\_collection\_extract}
\index{st\_cast}
...
\index{st\_intersection}
\index{st\_union}
\index{st\_difference}
\index{st\_sym\_difference}
...
\index{st\_union!n-ary}
\index{st\_intersection!n-ary}
\index{st\_difference!n-ary}
...
@fig-boxes) (right  shows the n-ary intersection: the 7
unique, non-overlapping geometries originating from intersection
of one, two, _or more_ geometries.

```{r fig-boxes, out.width='50%', fig.cap="left: three overlapping boxes -- how do we identify the small box where all three overlap? right: unique, non-overlapping n-ary intersections", eval=TRUE, fig=TRUE, echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-diff, out.width='50%', fig.cap="difference between subsequent boxes, left: in original order; right: in reverse order", eval=TRUE, fig=TRUE, echo=!knitr::is_latex_output()}
#| code-fold: true
par(mar = rep(.1, 4), mfrow = c(1, 2)) 
xg <- st_geometry(x)
...
```{r fig-rasterizeline, out.width = '60%', fig.cap = "rasterization artifact: as only top-left corners are part of the raster cell, only cells below the diagonal line are rasterized", echo=!knitr::is_latex_output()}
#| code-fold: true
library(stars)
...
on linear networks (@sec-pointpatterns).
