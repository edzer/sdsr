...
```{r fig-cube, out.width='70%', echo=!knitr::is_latex_output()}
...
time is split over multiple dimensions (e.g. year, day-of-year,
...
of spatial dimension (coordinate) variables that correspond to
e.g. the center or offset of spatial grid cells, and in addition
often store grid cell boundaries (needed to define rectlinear
dimensions or to disambiguate the relation of the coordinate variable
...
particular subregion, which is a more common operation. Fixing $x$
...
plotted (colored) in the space of one space and one time dimension.
...
e.g. applying a _temporal_ low-pass filter to every individual
...
* global warming (e.g. in degrees C per decade)
...
leads to highly flexible analysis possibilities, as e.g. shown
