...
e.g., the centre or offset of spatial grid cells, and in addition
often store grid cell boundaries (needed to define rectilinear
...
(e.g., a projection or its inverse). NetCDF files often provide both,
...
like "2021" for the full year, "2021-01" for the full month) or the time
...
at the `POINT` locations are _extracted_, e.g., by querying a pixel
...
* global warming (e.g., in degrees Celcius per decade)
...
leads to highly flexible analysis possibilities, e.g., shown
...
point pattern or trajectory data, e.g., offered by SciDB
