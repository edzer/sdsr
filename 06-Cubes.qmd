...
```{r fig-cube, out.width='70%', fig.cap="Raster data cube with dimensions latitude, longitude and time", echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-cube4d, fig.width = 8, fig.cap="Four-dimensional raster data cube with dimensions x, y, bands and time", echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-cube4d2, fig.height=10, fig.width=10, fig.cap="Four-dimensional raster data cube layed out flat over two dimensions", echo=!knitr::is_latex_output()}
#| code-fold: true
# flat ----
...
For more general cases like those in 
...
\filter{data cube!filter}

Data cubes can be sliced into sub-cubes by fixing a dimension at a
particular value. @fig-cube4filter  shows the sub-cubes
...
```{r fig-cube4filter, fig.width=10,fig.height=11, fig.cap="Data cube filtering by time, band or spatially", echo=!knitr::is_latex_output()}
#| code-fold: true
# filter ----
...
slice (i.e. every band/time combination), shown in 
@fig-cube4apply2.

```{r fig-cube4apply2, fig.height=6, fig.width=10, fig.cap="Low pass filtering of spatial slices", echo=!knitr::is_latex_output()}
#| code-fold: true
# apply ----
...
```{r fig-cube4apply1, fig.width=10, fig.height=7, fig.cap="Low pass filtering of time series", echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-cube4reduce, fig.width=10, fig.height=10, fig.cap="Reducing data cube dimensions", echo=!knitr::is_latex_output()}
#| code-fold: true
# reduce ----
...
```{r fig-cube4agg, fig.width=10, fig.height=8, fig.cap="Aggregating a raster data cube to a vector data cube", echo=!knitr::is_latex_output()}
#| code-fold: true
# aggregate ----
...
* estimating density maps from points or lines, e.g. with the number of flights passing by per week within a range of 1 km (@sec-pointpatterns)
...
```{r fig-aqdc, out.width='70%', fig.cap="Vector data cube with air quality time series", echo=!knitr::is_latex_output()}
#| code-fold: true
set.seed(1331)
library(stars)
library(colorspace)
