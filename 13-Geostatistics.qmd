...
website gives access to the data reported by European
member states.  We decided to download hourly (time series)
data, which is the data primarily measured.  A web form helps
convert simple selection criteria into an http `GET` request. The
form\footnote{https://eeadmz1-downloads-webapp.azurewebsites.net/}
was filled with Countries: DE; Cities: All; Pollutants: NO2; Dataset:
Primary validated data (E1a); Type: hourly data; Temporal coverage
Start: 01/01/2017, End: 01/01/2018; Download format: Parquet files.
This downloads a zip file with parquet files, one
per station.  Station metadata are downloaded from the [metadata
link](https://discomap.eea.europa.eu/App/AQViewer/index.html?fqn=Airquality_Dissem.b2g.measurements#)
shown at the bottom of the [EEA form
page](https://eeadmz1-downloads-webapp.azurewebsites.net/).
Below we assume the zip file is unpacked into subdirectory `aq`.
...
::: panel-tabset
#### R
```{r readallparquetfiles}
files <- list.files("aq/E1a", pattern = "*.parquet", full.names = TRUE)
# files <- setdiff(files, "aq/DataExtract.csv") # metadata file
library(arrow)
r <- lapply(files, function(f) read_parquet(f))
```
#### Python
```{python}
import glob
...
r = [pd.read_parquet(f) for f in files]
```
:::
...
r = [f.sort_values("Start") for f in r]
```
:::
...
names(r) <- sapply(r,
			   function(f) unique(substr(f$Samplingpoint, 11, 17)))
...
r <- lapply(r, function(f) xts(f$Value, f$t))
aq <- do.call(cbind, r)
aq[aq == -999] = NA
```
#### Python
...
#### Python
```{python}
aqsel = aq.loc[:, aq.isna().mean() < 0.25]
```
:::

Next, the station metadata was read 
...
read.csv("aq/DataExtract.csv") |>
	...
	filter(Country == "Germany",
           ...
           !duplicated(Air.Quality.Station.EoI.Code)) -> a2
```
#### Python
```{python}
rural = ["rural", "rural-regional", "rural-nearcity", "rural-remote"]
...
a2 = a2[~a2["Air Quality Station EoI Code"].duplicated()]
```
:::
...
#### R
```{r subseteuropean}
sel <- colnames(aqsel) %in% a2$Air.Quality.Station.EoI.Code
aqsel <- aqsel[, sel]
dim(aqsel)
```
#### Python
```{python}
aqsel = aqsel.loc[:, aqsel.columns.isin(a2["Air Quality Station EoI Code"])]
aqsel.shape
```
:::
...
de = gpd.read_file("data/de_nuts1.gpkg").to_crs(crs)
```
:::
...
#### Python
```{python}
# Python's spatial statistics ecosystem (libpysal/esda/PyKrige) has no
# cokriging/cosimulation demos comparable to gstat's; see the note near
# @sec-stgeos for the same gap in spatiotemporal kriging support.
```
:::
...
We can select the spatial locations for these stations by
...
Readers following the Python tabs should note that the rest of this section (@sec-stgeos) has no Python equivalent: **gstat**'s `variogramST()`, `vgmST()`, `fit.StVariogram()`, and `krigeST()` implement spatiotemporal variogram estimation, product-sum variogram model fitting, and spatiotemporal kriging, none of which are available in Python's spatial statistics ecosystem (`PyKrige` only fits purely spatial variograms; there is no widely-used Python package for spatiotemporal geostatistics comparable to **gstat**). Building an isolated `xarray` data cube to mirror the `stars` object above would have no Python consumer downstream, so it is not attempted either.
...
the relatively small number (76) of monitoring network stations
available.
