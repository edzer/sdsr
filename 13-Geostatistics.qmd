...
@sec-interpolation, this chapter works out a case study for
...
```{r fig-plotvariograms, echo=!knitr::is_latex_output(), out.height = '160%'}
#| fig.cap: "Spatiotemporal sample variogram for hourly NO$_2$ concentrations at rural background stations in Germany over 2027; in the right-hand side plot color corresponds to time lag."
v1 <- plot(v.st)
...
(fitProdSumModel <- fit.StVariogram(v.st, prodSumModel,
	...
	lower = rep(0.0001, 7)))
```
and shown in @fig-prodsummodelplot.
```{r fig-prodsummodelplot, fig.cap = "Product-sum model, fitted to the spatiotemporal sample variogram", echo=!knitr::is_latex_output()}
plot(v.st, fitProdSumModel, wireframe = FALSE, all = TRUE, scales = list(arrows=FALSE), zlim = c(0,150))
```
which can also be plotted as wireframes, shown in @fig-modelwire.
```{r fig-modelwire, fig.cap = "Wireframe plot of the fitted spatiotemporal variogram model", echo = !knitr::is_latex_output()}
...
```
```{r echo=FALSE}
save(list = "new_ts", file = "new_ts.RData")
```
where the results are shown in @fig-plotxts.
```{r fig-plotxts, fig.cap = "Time series plot of spatiotemporal predictions for two points", echo=!knitr::is_latex_output()}
...
```
```{r echo=FALSE}
save(list = "new_int", file = "new_int.RData")
```
```{r fig-plotspatiotemporalpredictions,fig.cap="Spatiotemporal predictions for four selected time slices", echo=!knitr::is_latex_output()}
library(viridis)
library(ggplot2)
...
```
and shown in @fig-plotspatiotemporalpredictions.
...
corresponding to those shown in @fig-plotspatiotemporalpredictions, and
compare the results.
1. Following the example in the demo scripts pointed
at in @sec-cokriging, carry out a cokriging
on the daily mean station data for the four days shown in
@fig-plotspatiotemporalpredictions.  What are the differences
