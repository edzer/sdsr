...
#### Python
```{python}
# xvec's GeometryIndex lets a dimension coordinate hold actual
...
Readers following the Python tabs should note that from here on, @sec-stgeos has no further Python equivalent: **gstat**'s `variogramST()`, `vgmST()`, `fit.StVariogram()`, and `krigeST()` implement spatiotemporal variogram estimation, product-sum variogram model fitting, and spatiotemporal kriging, none of which are available in Python's spatial statistics ecosystem (`PyKrige` only fits purely spatial variograms; there is no widely-used Python package for spatiotemporal geostatistics comparable to **gstat**). The `no2_st` vector data cube built above is real and usable, but nothing downstream in this section can consume it in Python.
