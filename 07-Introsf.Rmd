...
vector datacubes (Chapter \@ref(raster)), and has raster data stacks
...
R Package `sf` [@rjsf] and developed to succeed and, over time,
...
conversions using PROJ (figure \@ref(gdal-fig-nodetails)). These
libraries are interfaced using Rcpp [@eddelbuettel2013seamless].

## `sf` basics

Package `sf` represents sets of simple features are in `sf` objects,
a sub-class of a data frame or tibble. `sf` objects contain at least
one geometry list-column of class `sfc`, which for each element
contains the geometry as an R object that subclasses `sfg`.  An `sf`
object has the following meta-data held in attributes:
