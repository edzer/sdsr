...
relationship (section \@ref(agr)), held in attribute `agr`
...
(ref:sfobj) components of an `sf` object

```{r sfobj, out.width = '100%', echo = FALSE, fig.cap = '(ref:sfobj)', echo = FALSE}
...
```{r circr, out.width = '70%', echo=FALSE,fig.cap = "circular center region of the Landsat 7 scene (band 1)", message=FALSE}
...
e.g. to values for spatial polygons or lines (section
\@ref(vectordatacubes)). We can for instance compute the maximum
...
```{r rsample, out.height = '80%', fig.cap = "randomly chosen sample locations for training data; red: water, green: land", echo=!knitr::is_latex_output(), message=FALSE}
...
```{r lda, out.width = '70%', fig.cap = '(ref:cap)',  echo=!knitr::is_latex_output()}
...
```{r firststars,fig.cap="six 30m Landsat bands downsampled to 90m for Olinda, Br.", echo=!knitr::is_latex_output(), message=FALSE}
plot(r)
```
...
```{r starsrgb, out.width = '100%', fig.cap = "two color composites", echo=!knitr::is_latex_output(), message=FALSE}
par(mfrow = c(1, 2))
...
```{r airmap, out.width = '70%', fig.cap="locations of PM$_{10}$ measurement stations, showing mean values", echo=!knitr::is_latex_output()}
...
and plot it with
```{r odm33, out.width = '80%', fig.cap = "OD matrix sliced for destination zone 33, by transportation mode"}
...
```{r odjoined, out.height = '90%', fig.cap = "total commutes, summed by origin (left) or destination (right)."}
...
```{r odbykm, out.height = '90%', fig.cap = "total commutes per square km, by area of origin (left) or destination (right)", message=FALSE}
library(units)
...
As mentioned in section \@ref(wkt2), the definition of `EPSG:4326`,
```{r echo=FALSE}
...
e.g. with plotting data.  At the time of writing this, the `plot`
method for `sf` objects respects the axis order flag and will swap
coordinates using the transformation pipeline `"+proj=pipeline
+step +proj=axisswap +order=2,1"` before plotting them, but
e.g. `geom_sf()` in `ggplot2` has not been modified to do this.
As mentioned earlier, the ambiguity of `EPSG:4326` is resolved by
replacing it with `OGC:CRS84`.
