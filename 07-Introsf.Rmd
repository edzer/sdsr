#directly (PART) R for Spatial Data Science {-}
...
```{r circr, echo=FALSE,fig.cap = "circular center region of the Landsat 7 scene (band 1)", message=FALSE}
...
```{r rsample,fig.cap = "randomly chosen sample locations for training data; red: water, green: land", echo=!knitr::is_latex_output(), message=FALSE}
...
```{r firststars,fig.cap="6 30m Landsat bands downsampled to 90m for Olinda, Br.", echo=!knitr::is_latex_output(), message=FALSE}
plot(r)
```
...
```{r odbykm, fig.cap = "total commutes per square km, by area of origin (left) or destination (right)", message=FALSE}
library(units)
