...
```
/* RSB consider adding figure something like:
plot(st_geometry(nc7))
plot(st_geometry(nc5), add=TRUE, border="brown")
text(xy, labels=1:nrow(nc7), cex=0.8, col="blue")
and explain the link to the sparse representation */
...
/* RSB sgbp not mentioned in 17 */

### Subsetting

/* RSB consider moving to before 7.1.3 and use to create subsets */
...
/* RSB should sf_use_s2(TRUE) be in this block rather than the next? */
```{r}
...
/* RSB patch sf source proj.4 -> PROJ ? */
```{r}
