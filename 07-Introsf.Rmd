...
sets this would yield a very large matrix, mostly filled with `FALSE`
values and for that reason a sparse representation is returned by
default:
```{r}
nc5 = nc %>% head(5)
nc7 = nc %>% head(7)
...
lengths(i)
```
and the other way around it is
```{r}
lengths(t(i))
```
...
* selection with a spatial (`sf`, `sfc` or `sfg`) object as first argument leads to
  ...
  parameter `op` to a binary predicate function, such as `st_covers`.
...
serves illustration, it was _not_ used in this analysis.
