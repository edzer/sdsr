...
of stars objects just like `apply` does this to matrices. For
instance, to compute for each pixel the mean of the 6 band values
we can do
```{r}
...
```
which is more convenient if there are a large amount of bands,
but it has a much slower implementation.
...
o$sum_km = o$sum / a
d$sum_km = d$sum / a
plot(c(o["sum_km"], d["sum_km"], along = list(od = c("origin", "destination"))), logz = TRUE)
```
