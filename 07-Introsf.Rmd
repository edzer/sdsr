...
packages received support from the R Consortium as well as a strong
of community engagement. The packages were designed to work together.
...
spatial software with the tidyverse.
...
dimensions (e.g. time), `offset` and `delta` are not `NA` and values
...
```{r rsample,fig.cap = "randomly chosen sample locations for training data; red: water, green: land"}
plot(r[,,,1], reset = FALSE)
col = rep("green", 20)
col[c(8, 14, 15, 18, 19)] = "red"
st_as_sf(e) %>% st_coordinates() %>% text(labels = 1:20, col = col)
```
From this figure, we read "by eye" that the points 8, 14, 15,
18 and 19 are on water, the others on land. Using a linear
discriminant ("maximum likelyhood") classifier, we find model
predictions as shown in figure \@ref(fig:lda).

(ref:cap) Linear discriminant classifier for land/water, based on training data of figure \@ref(fig:rsample)

```{r lda,fig.cap='(ref:cap)'}
rs = split(r)
trn = st_extract(rs, pts)
trn$cls = rep("land", 20)
trn$cls[c(8, 14, 15, 18, 19)] = "water"
library(MASS, warn.conflicts = FALSE) # masks "select"
model = lda(cls ~ ., st_set_geometry(trn, NULL))
pr = predict(rs, model)
detach(2) # we remove MASS from search path, so "select" is again dplyr's
