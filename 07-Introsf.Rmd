...
Inteded to succeed and replace R packages `sp`, `rgeos` and the
...
modern versions of the open source geospatial software stack, and
to allow for integration of R spatial software with the tidyverse.

To do so, R package `sf` provides simple features access
[@herring2011opengis], natively, to R. It provides an interface
to several `tidyverse` packages, in particular by `ggplot2`,
`dplyr` and `tidyr`. It can read and write data through GDAL, use
geometrical operations using GEOS (for Cartesian coordinates)
or s2geometry (for ellipsoidal coordinates), and carry out
coordinate transformations or conversions using PROJ (figure
\@ref(fig:gdal-fig-nodetails)). External C++ libraries are interfaced
using `Rcpp` [@eddelbuettel2013seamless].
...
Geometry list-columns act as a simple variable in a `data.frame`
or tibble, but have a more complex structure than e.g. numeric or
...
* for each non-geometry variable, the attribute-geometry relationships, held in attribute `agr`
...
* the precision held in attriute `precision`
...
More typically, spatial data in R are read from an external source,
which can be
...
\@ref(de9im)) take two sets of feature (geometries) and return for
...
```
which we could transform into a logical matrix by
```{r}
as.matrix(i)
```
...
Chapter \@ref(area) explains how `sgbp` objects can be converted into
`nb` objects for use in spatial regression models.
...
`sf` objects, with a few additional features, in particular

* the `drop` argument when set to `TRUE` drops the geometry column when not selected,
  meaning that a `data.frame` is returned; the default is `FALSE` meaning that the
  geometry column is _always_ selected, and an `sf` object is returned.
* selection with a spatial (`sf`, `sfc` or `sfg`) object as first argument leads to
  ...
  object; other predicates than _intersects_ can be chosen by setting
...
together, described e.g. in [@r4ds]. Package `sf` has tidy-verse
style read and write functions, `read_sf` and `write_sf`, that
return a tibble rather than a `data.frame`, do not print any output,
and overwrite by default. 
...
The `summarise` method for `sf` has two special arguments:
...
and in addition a buffer around the county; note that this buffer
serves illustration, it was _not_ used to select the counties. 
...
triangular.  Chapter \@ref(spatstat) explains how spatial sampling
(or point pattern generation) methods available in package `spatstat`
...
Athough package `sp` has always had limited support for raster
data, over the last decade R package `raster` has clearly been
...
its successor, `terra`) is that of a 2D regular raster, or a set of
raster layers (a "raster stack"). This aligns with the classical
static "GIS world view", where the world is modelled as a set of
layers, each representing a different theme. A lot of data available
today however is dynamic, and comes as time series of rasters or
raster stacks. A raster stack does not meaningfully reflect this,
requiring the user to keep a register of which layer represents what.
...
We introduce package `stars` here for analysing raster and vector
...
Raster data typically are read from a file. We can read an example
...
```
which is small enough to be printed here as a `data.frame`. In these
...
resample; resampling always goes at the cost of some loss of data
...
\@ref(wkt2)) or PROJJSON.

The object returned contains two fields, `wkt` with the WKT2
...
WKT2 in a `crs` object to a proj4string using the `$proj4string`
method, as in
```{r}
x = st_crs("OGC:CRS84")
x$proj4string
...
it choosed the options with the highest accuracy. More finegrained
...
```
which can be read, and even edited (with care!) to add projections.
