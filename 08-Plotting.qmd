...
maps are good and which are bad, but rather point out a number of
possibilities how to create them, and point out challenges along
...
the world "as it is", e.g. as seen from space (@fig-world, right).
...
latitude linearly to the $x$ and $y$ axis, keeping an aspect ratio of
1. Were we to do this for smaller areas not on the equator, then it
makes sense to choose a plot ratio such that one distance unit E-W
equals one distance unit N-S on the center of the plotted area,
...
scale (i.e., where one length unit N-S equals one length unit E-W),
which was here chosen at the middle of the bounding box latitudes
...
should be preferred to a decision process on its own.  For global
maps however in almost all cases, equal area projections are to be
preferred over plate carr\'{e}e or web Mercator projections.
...
latitude or longitude. On @fig-first-map  a graticule is drawn in
grey, on @fig-firstgather  in white.  Graticules are often drawn in
...
Graticules, by that, also shed light on which projection was used:
...
meridians, equal area may have curved meridians.
...
* given a geometry and an attribute, the geometry is coloured according to
  ...
  or `logical` attributes and a continuous scale otherwise, and a color key
  is added
...
in two. If one wants to _add_ to an existing plot having a colour
...
```{r fig-figreset}
#| fig.cap: "Annotating base plots that have a legend"
library(sf)
...
system of the first set. To get further control the "base"
...
are given in @sec-area - @sec-spatglmm.
