...
#| fig.cap: "Earth country boundaries; left: mapping long/lat linearly to x and y (plate carrée); right: as seen from an infinite distance (orthographic)"
#| out.width: 90%
# Python has no mature equivalent of sf's s2-based spherical
# intersection used here to clip countries to the visible
# hemisphere. This approximates it: a country is kept only if every
# vertex is within ~85 degrees great-circle distance of the centre
# point, which whole-country-excludes (rather than partially clips)
# countries straddling the horizon (e.g. Russia, Antarctica).
import geopandas as gpd
import numpy as np
import matplotlib.pyplot as plt

w = gpd.read_file("data/ne_110m_admin_0_countries.shp")
lon0, lat0 = np.radians(-30), np.radians(-10)

def angular_dist(lon, lat):
    lon, lat = np.radians(lon), np.radians(lat)
    cos_d = (np.sin(lat0) * np.sin(lat) +
             np.cos(lat0) * np.cos(lat) * np.cos(lon - lon0))
    return np.arccos(np.clip(cos_d, -1, 1))

def all_visible(geom):
    polys = [geom] if geom.geom_type == "Polygon" else list(geom.geoms)
    xs, ys = [], []
    for p in polys:
        cxs, cys = p.exterior.coords.xy
        xs.extend(cxs)
        ys.extend(cys)
    return np.all(angular_dist(np.array(xs), np.array(ys)) < np.radians(85))

visible = w[w.geometry.apply(all_visible)]
ortho = f"+proj=ortho +lat_0={np.degrees(lat0)} +lon_0={np.degrees(lon0)}"

fig, axes = plt.subplots(1, 2, figsize=(10, 5))
w.boundary.plot(ax=axes[0], color="black", linewidth=0.5)
axes[0].set_axis_off()

visible.to_crs(ortho).plot(ax=axes[1], facecolor="lightgrey",
...
fig, ax = plt.subplots()
nc.plot(column="BIR74", ax=ax, legend=True)
buf = nc.iloc[[0]].to_crs("EPSG:32119").buffer(10_000).to_crs(nc.crs)
buf.boundary.plot(ax=ax, color="red", linewidth=2)
ax.set_axis_off()
plt.show()
```
:::
...
#### Python
```{python}
#| fig.cap: "Simple facet raster plot with xarray"
import rioxarray as rxr

da = rxr.open_rasterio("data/L7_ETMs.tif")
da.plot.imshow(col="band", col_wrap=3, cmap="viridis", add_colorbar=True)
```
:::
...
Readers following the Python tabs should note that this whole section has no Python equivalent: **tmap** is a self-contained grammar-of-graphics-style package with no counterpart in the Python spatial ecosystem (Python maps are generally built directly with **geopandas**/**matplotlib**, as in the sections above, or with dedicated interactive libraries like **folium**), so none of the `tmap`-specific code below is translated.
