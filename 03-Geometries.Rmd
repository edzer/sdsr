...
Having learned how we describe coordinates systems, we can define
...
* rasters, rectangular subdivisions of space
* networks
...
```{r sfgeometries,fig.cap = "sketches of the main simple feature geometry types", echo=TRUE}
library(sf)
par(mfrow = c(2,4))
par(mar = c(1,1,1.2,1))
...
section @ref(axisorder)).

### Simple and valid geometries, ring direction {#valid}
...
Valid polygons and multipolygons obey the following properties:
...
on the sphere, the "clockwise" concept does not work well: if we take
...
in `LINESTRING`s. These become however invalid (or "non-simple")
once the trajectory self-intersects, as only `X` and `Y` are
considered for self-intersections.
...
```
but it seems that they all point to the same empty set.
...
`CIRCULASTRING`, `COMPOUNDCURVE` and `CURVEPOLYGON` are not
...
between R package `sf` and the GDAL, GEOS, liblwgeom and s2geometry
libraries.

## Operations on geometries {#opgeom}
...
operations entirely focusing on _geometrical_ properties.  Chapter,
...
The predicates `is_simple`, `is_valid`, and `is_empty` returns
...
### Binary predicates and DE-9IM {#de9im}

The Dimensionally Extended nine-Intersection Model (DE-9IM,
[@de9im1; @de9im2]) is a model that helps describing the qualitative
...
```{r de9im,echo=TRUE,fig.cap = "DE-9IM: intersections between the interior, boundary and exterior of a polygon (rows) and of a linestring (columns) indicated by red"}
library(sf)
...
_mask string_; e.g. the string `"*0*******"` would evaluate `TRUE`
...
symbol `T` stands for any intersection (of dimensionality 0, 1 or 2).
...
|--------------------------|-----------------------------------------------------------------|----------------|
|`equals`     |Two geometries A and B are topologically identical|
...
|`equals` |A and B are geometrically equal; node order number of nodes may differ; identical to A contains B AND A within B|
...
|`relate`|given a mask pattern, returns whether A and B adhere to this pattern| |
...
|measure              |returns                                                       |
|---------------------|--------------------------------------------------------------|
|`dimension`|0 for points, 1 for linear, 2 for polygons, `NA` for empty geometries|
|`area`|the area of a geometry|
|`length`|the length of a linear geometry|
|`geohash`|the [geohash](https://en.wikipedia.org/wiki/Geohash) for a geometry|
|`geometry_type`|the type of a geometries|

### Binary Measures

`distance` returns the distances between pairs of geometries.
`relate` (without mask) returns the relation pattern, a qualitative
description of the geometrical relationship between two geometries
explained in section \@ref(de9im).
...
|`convex_hull`|that forms the convex hull of the input geometry (figure \@ref(fig:vor))|
...
|`transform`|transformed to a new coordinate reference system (chapter \@ref(rs))|
|`triangulate`|with triangulated polygon(s)|
|`voronoi`|with the voronoi tesselation of an input geometry (figure \@ref(fig:vor))|
...
```{r vor, echo=TRUE, out.width='60%', fig.cap="left: convex hull (red) around a polygon (black); right: voronoi diagram (red) from a `MULTIPOINT` (black)"}
...
|`sym_differenec`   |the combinations of the geometries after removing where they overlap|`%/%`|
...
```{r boxes, out.width='50%', fig.cap="left: three overlapping boxes -- how do we identify the small box where all three overlap? right: unique, non-overlapping n-ary intersections", echo=TRUE, eval=TRUE, fig=TRUE}
...
...\}$. This is shown in figure \@ref(fig:diff), left for the original
...
```{r diff, out.width='50%', fig.cap="difference between subsequent boxes, left: in original order; right: in reverse order", echo=TRUE, eval=TRUE, fig=TRUE}
par(mar = rep(.1, 4), mfrow = c(1, 2)) 
xg = st_geometry(x)
...
is found [here](https://github.com/r-spatial/sf/issues/275).


## Coverages: tesselations and rasters {#coverages}

(FIXME: discuss DE9-IM and uniquely assigning _all_ points!!
"Spatial predicates are topologically-invariant binary spatial relations based on the DE-9IM"
invariant under homeomorphisms. Projections are, in general, no homeomorphisms.
(mappings that preserve all the topological properties of a given space)
)

OGC [@ogccov] defines a _coverage_ as a ``feature that acts as a
function to return values from its range for any direct position
within its spatiotemporal domain''. Having a _function_ implies that
for every spatiotemporal "point" (spatial point and moment in time)
of the domain we have _single_ value for the range. This is a very
common situation for many spatiotemporal phenomena, a few examples
can be given:

* boundary disputes aside, every point in a region (domain) belongs to a single adminstrative unit (range)
...
(A caveat here is that because measurement always takes time
...
do not overlap, or that there are no gaps between them.
...
setting thresholds and handling gaps and overlaps.

### Raster tesselations

A tesselation is a subdivision of a space (area, volume) into
...
tesselate each spatial dimension $d$ into regular cells,
...
Tesselating the time dimension with left-closed, right-open intervals
is very common, and reflects the implicit assumption underlying
time series packages such as `xts` in R, where time stamps indicate
...
a space-time _vector datacube_.  Raster data and data cubes are
discussed in chapter \@ref(raster).
...
Several R packages (`osmar`, `stplanr`, `sfnetworks`) have functionality
