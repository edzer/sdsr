...
```{r sfgeometries,fig.cap = "sketches of the main simple feature geometry types", echo=!knitr::is_latex_output()}
library(sf)
par(mfrow = c(2,4))
par(mar = c(1,1,1.2,1))
...
```{r de9im, fig.cap = "DE-9IM: intersections between the interior, boundary and exterior of a polygon (rows) and of a linestring (columns) indicated by red", echo=!knitr::is_latex_output() }
library(sf)
...
```{r vor, out.width='60%', fig.cap="left: convex hull (red) around a polygon (black); right: voronoi diagram (red) from a `MULTIPOINT` (black)", echo=!knitr::is_latex_output()}
...
|`sym_difference`   |the combinations of the geometries after removing where they overlap|`%/%`|


### N-ary Transformers {#nary}
...
```{r boxes, out.width='50%', fig.cap="left: three overlapping boxes -- how do we identify the small box where all three overlap? right: unique, non-overlapping n-ary intersections", eval=TRUE, fig=TRUE, echo=!knitr::is_latex_output()}
...
```{r diff, out.width='50%', fig.cap="difference between subsequent boxes, left: in original order; right: in reverse order", eval=TRUE, fig=TRUE, echo=!knitr::is_latex_output()}
par(mar = rep(.1, 4), mfrow = c(1, 2)) 
xg = st_geometry(x)
