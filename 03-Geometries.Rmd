...
sections \@ref(proj) and \@ref(axisorder)).
...
usually be repared to make them valid.
...
communications between R package `sf` and the GDAL, GEOS, liblwgeom
...
* **measures**: a quantity (e.g. a numeric value with measurement unit), or
...
symbol `T` stands for any non-empty intersection (of dimensionality
0, 1 or 2).
...
a _function_ implies that for every "point", i.e. every combination
of spatial point and a moment in time of the spatiotemporal domain,
we have _single_ value for the range. This is a very common situation
...
* simple features have no way of assigning points _on_ the boundary of two adjacent polygons uniquely to a single polygon, which introduces ambiguity in terms the interpretation as coverage.
...
way around: reconstructing topology from a set of polygons typcially
...
Where in arbitrary polygon tesselations the assignment of points
...
in regular tesselations removes this ambiguity. This means that for
rasters with negative $\delta$ values for the $y$-coordinate and
postive for the $x$-coordinate, only the top-left corner point
is part of each raster cell. An artifact resulting from this is
shown in figure \@ref(fig:rasterizeline).

```{r rasterizeline, fig.cap = "rasterization artifact: as only top-left corners are part of the raster cell, only cells below the diagonal line are rasterized", echo=!knitr::is_latex_output()}
library(stars)
...
be combined: one could use simple feature polygons to tesselate
space, and combine this with a regular tesselation of time in order
to cover a space-time _vector data cube_.  Raster and vector data
cubes are discussed in chapter \@ref(raster).

As mentioned above, besides square cells the other two shapes
that can lead to regular tesselations of $R^2$ are triangles
