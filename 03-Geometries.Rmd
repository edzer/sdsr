...
can be described in these spaces. This chapter will mostly explain the
...
(GDAL, GEOS, liblwgeom) and spatial databases because it is fast
...
```{r}
rbind(c(0,0), c(1,1), c(1,0), c(0,1)) %>% st_linestring() %>% st_cast("MULTIPOINT")
```
but if we want to have a set of points, we need to work with sets (section \@ref(sfc)) first, because
...
```
which are used to record _for the whole set_:
...
`sf` objects are `tibble` or `data.frame` objects with feature geometries
...
negative for the $y$-axis (Northing), indicating that raster row
numbers increasing Southwards correspond to $y$-coordinates
...
Geometries on the sphere are geometries made from geographic
...
Cartesian space.  Technically speaking, a straight line between
...
Package `sf` comes with a large number of functions that work both
