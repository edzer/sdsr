...
n = 30
np.random.seed(13531)  # Remove this line to create another random sequence
x = np.random.uniform(0, 1, n)
y = np.random.uniform(0, 1, n)
xy = gpd.GeoDataFrame({'geometry': [Point(xy) for xy in zip(x, y)]})
```
:::
...
#### Python
```{python}
# Plot random points within observation windows
fig, axs = plt.subplots(1, 2, figsize=(10, 4))
for ax, window, title in zip(axs, [w1, w2], ["Window 1", "Window 2"]):
    ax.set_title(title)
    ax.add_patch(plt.Rectangle((window.bounds[0], window.bounds[1]),
...
# Using previously created random points (xy)
# Define observation window (bounding box)
bbox = Polygon([(0, 0), (1, 0), (1, 1), (0, 1)])

# Create a GeoDataFrame as a point pattern
### EP: the following does not make sense IMO:
# xy['window'] = bbox
# print(xy.head())
```
:::
...
plt.imshow(Z, origin='lower', aspect='auto', extent=(x_grid.min(), x_grid.max(), y_grid.min(), y_grid.max()), cmap='Blues')
...
x_grid_size = x_grid[1] - x_grid[0]
y_grid_size = y_grid[1] - y_grid[0]
pixel_area = x_grid_size * y_grid_size

# Calculate the total area under the KDE surface
total_area = (Z * pixel_area).sum()
print(f"Total area under the KDE surface: {total_area}")
```
:::
