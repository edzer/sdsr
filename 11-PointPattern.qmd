...
@STOYAN2017125. An R package for analysing spatiotemporal 
point processes is discussed in @stpp.
...
```{r fig-pp1, fig.height=3.5, echo=!knitr::is_latex_output()}
...
an appropriate quadrat layout (a 3 x 3 layout is shown in 
@fig-quadrat)
```{r fig-quadrat, fig.height=3.5, fig.cap = "3 x 3 quadrat counts for the two point patterns", echo = !knitr::is_latex_output()}
...
```
```{r fig-bwdiggle, fig.height = 3.5, fig.cap = "Kernel densities for both point patterns", echo=!knitr::is_latex_output()}
...
```{r fig-ppm, fig.cap = "Predicted densities of a ppm model", echo=!knitr::is_latex_output()}
plot(m, se = FALSE)
```
...
nrow(th)
```
```{r fig-rThomas, fig.height = 4, fig.cap = "Thomas process with mu = 3 and scale = 0.05", echo=!knitr::is_latex_output()}
par(mar = rep(0, 4))
plot(w2)
plot(th, add = TRUE)
```
...
## Simulating points on the sphere
...
```{r fig-srsglobe, fig.cap = "Points randomly sampled over the oceans", echo=!knitr::is_latex_output()}
