...
```{r largest, fig.cap="example of `st_join` with `largest = TRUE`: the label of the polygon in the top figure with the largest intersection with polygons in the bottom figure is assigned to the polygons of the bottom figure`", echo=FALSE}
...
Unioning aggregated geometries resolve for instance internal polygon
...
```{r union, fig.cap="left: invalid `MULTIPOLYGON` with two external rings with common boundary, right: valid `POLYGON` obtained after unioning the geometry left", echo=FALSE}
...
```
```{r intersection, fig.cap="left: overlapping geometries (d2: red); right: intersection areas (i: grey)"}
