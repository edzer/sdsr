...
GEOGCRS["WGS 84 (CRS84)",
    ...
    ID["OGC","CRS84"]]
```
This shows a coordinate system with the axis order _longitude,
latitude_ that can be used to replace `EPSG:4326` when one wants
unambiguously "traditional" (GIS) axis order.  The _ensemble_ of
WGS84 ellipsoids listed represents its various updates over time.
Ambiguity about _which_ of these ensemble members a particular
dataset should use leads to an uncerainty of several meters.
