...
system so that we have an idea what exactly coordinates of a
...
units; exampes are speed ($\mbox{m}~\mbox{s}^{-1}$) and density
($\mbox{kg}~\mbox{m}^{-3}$).
...
Many data variables have units that are not expressable as SI base
...
can meaningfully measure other absolute space-time points: a datum.
...
```{r polar, out.width = '50%', fig.cap="Two-dimensional polar (red) and Cartesian (blue) coordinates", Eva=TRUE, fig=TRUE, echo=TRUE}
par(mar = rep(0,4))
...
```{r sphere,echo=TRUE,fig.cap="Cartesian geocentric coordinates (left) measure three distances, ellipsoidal coordinates (right) measure two angles, and possibly an ellipsoidal height"}
...
```{r ellipse, out.width = '50%', fig.cap="Angles on an ellipse: geodetic (blue) and geocentric (red) latitude", eval=TRUE, fig=TRUE, echo=TRUE}
par(mar = rep(0,4))
x = 4
y = 5/8 * sqrt(48)
...
one projection to the other without changing datum is called
...
from one datum to another may be time-dependent. Eartquakes are a
cause for more local changes in coordinates.
...
for cartographic projections. It came with a few command line tools
for direct and inverse projections, and could be linked to software
...
In the early 2000s, amongst others motivated by the rise of GPS,
the need for datum transformations increased and PROJ was extended
...
would come with a fields like
```
+ellps=bessel +towgs84=565.417,50.3319,465.552,-0.398957,0.343988,-1.8774,4.0725
```
indicationg another ellipse, as well as the seven (or three)
...
Along with PROJ came a set of databases with known (registered)
...
distributed as part of PROJ: such grids are raster maps that provide
for every location pre-computed values for the shift in longitude
and latitude for a particular datum transformation.

This way, every coordinate transformation had to go through a
...
In 2018, after a succesful "GDAL Coordinate System Barn Raising"
...
with the library but are accessible from a CDN; PROJ allows to switch on
network access to access these grids: only the section of the grid needed
is accessed, and is stored in a cache on the user's machine
...
transformation can be as good as 1 metre. An interesting point is
the last: Where we could safely assume for many decades that spatial
data with ellipsoidal coordinates would have axis order (longitude,
latitude), this is no longer the case. We will see in section
\@ref(axisorder) how to deal with this.

### WKT2

@lott2015 describes a standard for encoding coordinate reference
system, as well as transformations between them using _well known
text_; the standard (and format) is referred to as WKT2. As mentioned
above, GDAL and PROJ fully support this encoding.  An example of
WKT2 for CRS `OGC:CRS84` is:
