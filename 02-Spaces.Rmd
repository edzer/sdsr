...
with angles in degrees, and geocentric coordinates
```{r echo=FALSE}
p = st_as_sfc("POINT(60 47)", crs = 4326) %>% st_transform("+proj=geocent")
p[[1]]
```
...
passing through the poles, the latitude measure $\phi$ is the one
used when no further specification is given; it is also called
_geographic latitude_. The latitude measure $\psi$ is called the
...
the Earth in different are being used. Such ellipsoids are called
...
To show that the different distance measures (ellipsoidal, spherical, straight line)
actually give differnt values, we computed them for the distance Berlin - Paris:
```{r echo=FALSE}
pts = st_sfc(st_point(c(13.4050, 52.5200)), st_point(c(2.3522, 48.8566)), crs = 'EPSG:4326')
sf_use_s2(FALSE)
d1 = c(ellipsoidal = st_distance(pts)[1,2])
sf_use_s2(TRUE)
# or, without s2, st_distance(st_transform(pts, "+proj=longlat +ellps=sphere"))
d2 = c(spherical = st_distance(pts)[1,2])
p = st_transform(pts, "+proj=geocent")
d3 = c(straight_line = units::set_units(sqrt(sum(apply(do.call(cbind, p), 1, diff)^2)), m))
c(d1,d2,d3)
```
<details>
<summary style="color:blue;">Click for Code</summary>
...
inside and an outside. On $S^2$, any polygon on the sphere divides
the sphere in two parts: which of these two is to be considered
...
daily, weekly, and yearly signals. This decomposes time essentially in two
or more components, with some of them being cyclic.
