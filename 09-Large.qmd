...
#### Python
```{python}
# geopandas' read_file() has a `bbox` argument that GDAL applies the
# same way as sf's wkt_filter: only features whose bounding box
# intersects the given box are returned (out of 100)
import geopandas as gpd
from shapely.geometry import box

bb = box(-82, 36, -80, 37)
len(gpd.read_file("data/nc.gpkg", bbox=bb))
```
:::
...
#### Python
```{python}
# geopandas' read_file() accepts a full OGR SQL statement through
# the `sql` argument, passed on to GDAL just like sf's `query`
q = 'select BIR74,SID74,geom from "nc.gpkg" where BIR74 > 1500'
len(gpd.read_file("data/nc.gpkg", sql=q))
```
:::
...
#### Python
```{python, eval=FALSE}
# geopandas.read_postgis() takes a raw SQL query and a DB-API/SQLAlchemy
# connection; like has_PG above, this requires database credentials not
# available in this environment, so it is not executed here
import os
import geopandas as gpd
from sqlalchemy import create_engine

engine = create_engine(
    f"postgresql://{os.environ['DB_USERNAME']}@{os.environ['DB_HOST']}/postgis"
)
gdf = gpd.read_postgis(
    "select BIR74, wkb_geometry from nc limit 3", engine, geom_col="wkb_geometry"
)
len(gdf)
```
:::
...
q = ("SELECT BIR74, wkb_geometry FROM nc WHERE "
     "ST_Intersects(wkb_geometry, 'SRID=4267;POINT (-81.50 36.43)')")
len(gpd.read_postgis(q, engine, geom_col="wkb_geometry"))
```
:::
...
#### Python
```{python, eval=FALSE}
# Python has no dbplyr-style lazy table abstraction that translates
# dataframe verbs into SQL; the closest equivalent is to formulate SQL
# directly (as above) or use an ORM/query-builder such as SQLAlchemy
# Core or ibis, neither of which are geopandas-integrated the way
# dbplyr is with sf
nc_db = "nc"  # table name, used directly in SQL below
```
:::
...
```{python, eval=FALSE}
gpd.read_postgis(
    "SELECT * FROM nc WHERE ST_Intersects("
    "wkb_geometry, 'SRID=4267;POINT (-81.50 36.43)')",
    engine, geom_col="wkb_geometry",
)
gpd.read_postgis(
    "SELECT * FROM nc WHERE ST_Area(wkb_geometry) > 0.1 LIMIT 3",
    engine, geom_col="wkb_geometry",
)
```
:::
...
#| fig.cap: "OpenStreetMap vector data"
#| fig.height: 5
# the GDAL OSM vector driver (behind read_sf()/st_read()) is also
# used by geopandas/pyogrio, so the same file and layer names apply
import geopandas as gpd
import matplotlib.pyplot as plt
from shapely.geometry import box

o = gpd.read_file("data/ms.osm", layer="lines")
p = gpd.read_file("data/ms.osm", layer="multipolygons")

fig, ax = plt.subplots(figsize=(6, 5))
bbox = gpd.GeoSeries([box(7.595, 51.969, 7.598, 51.970)], crs="OGC:CRS84")
bbox.boundary.plot(ax=ax, color="black", linewidth=2, linestyle="--")
o.plot(ax=ax, color="black", linewidth=2)
p.plot(ax=ax, color="#88888888", edgecolor="none")
plt.show()
```
:::
...
#### Python
```{python}
# rioxarray/rasterio understand the same GDAL /vsizip/ and
# SENTINEL2_L1C: prefixes used by sf/stars, since both ultimately
# call into GDAL. Passing chunks=True makes xarray back the array
# with dask instead of numpy, giving the same "proxy" behaviour as
# read_stars(..., proxy = TRUE): only metadata is read up front, no
# pixel values yet
import os
...
granule = os.path.join(
    os.path.expanduser("~/R/x86_64-pc-linux-gnu-library/4.4/starsdata"), f
)
os.path.getsize(granule)
base_name = os.path.basename(granule).removesuffix(".zip")
s2 = (f"SENTINEL2_L1C:/vsizip/{granule}/{base_name}"
	  ".SAFE/MTD_MSIL1C.xml:10m:EPSG_32632")
p = rxr.open_rasterio(s2, chunks=True)
type(p.data)  # dask.array.core.Array: not yet computed
```
:::
...
#### Python
```{python}
p2 = p * 2  # still a dask array: the multiplication is postponed too
type(p2.data)
```
:::
...
#| fig.cap: "Downsampled 10 m bands of a Sentinel-2 scene"
#| code-fold: true
# xarray/dask have no notion of a plotting device resolution the way
# R's plot.stars_proxy does, so a downsample factor is chosen
# explicitly here for a representative device size (see also below)
import numpy as np
import matplotlib.pyplot as plt

target_px = (700, 700)
ds = int(np.floor(np.sqrt(np.prod(p.shape[1:]) / np.prod(target_px))))

small = p.isel(y=slice(0, None, ds), x=slice(0, None, ds)).compute()
fig, axes = plt.subplots(1, small.sizes["band"], figsize=(12, 3.5))
for i, ax in enumerate(axes):
    ax.imshow(small.isel(band=i).values, cmap="gray")
...
```{python}
#| code-fold: true
# recomputed from the same formula used above, since Python has no
# equivalent way to query a graphics device's pixel resolution the
# way R's dev.size("px") does in a headless render
ds
```
:::
...
#### Python
```{python, eval=FALSE}
# xarray reads Zarr natively (no GDAL /vsicurl/ prefix needed): a
# plain https:// URL, or an fsspec-style URL for other backends, is
# enough. This also means Python's route to cloud-native Zarr access
# doesn't go through GDAL's multi-dimensional array API at all
dsn = "https://ncsa.osn.xsede.org/Pangeo/pangeo-forge/gpcp-feedstock/gpcp.zarr"
```
:::
...
r = xr.open_zarr(dsn).isel(time=slice(0, 10))
r.rio.bounds()  # via rioxarray, once a CRS/spatial dims are set
```
:::
In this example, 
