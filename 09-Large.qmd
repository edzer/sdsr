...
Terabyte- and Petabyte-sized data sets.  Besides size considerations,
...
* using compression, such as the LERC (Limited Error Raster
  Compression) algorithm used for cloud-optimized GeoTIFF, or the 
...
* optimizing data access with particular cloud storage or object
  storage protocols in mind.

It should be noted that there are no silver bullets in this area:
optimizing storage for one particular access way will lead to slow
...
leads to low storage costs but to higher processing costs when
reading, because of the decompression involved.
...
obtained by assuming the coordinates are projected, where they
are ellipsoidal.
...
always be the most efficient way of handling resources. 
...
storage; the difference seems to be that (Geo)Parquet is more
oriented towards persistent storage, where (Geo)Arrow is more
oriented to fast access and fast computation; Arrow can for instance
be both an in-memory and an on-disk format.
...
method works:  in the case of plotting large rasters, it downsamples
the array before it plots, drastically saving time.  The degree
of downsampling is derived from the plotting region size and the
...
```{r fig-plotp, fig.cap="downsampled 10 m bands of a Sentinel-2 scene", echo=!knitr::is_latex_output(), message=FALSE}
plot(p)
```
...
\index{cloud-optimized GeoTIFF}
\index{GeoTIFF!cloud-optimized}

A format like "cloud-optimized GeoTIFF" (COG) has been specially
...
Function `stars::read_mdim` has options for reading sub-cubes by
specifying offset and size of the index in each dimension, and a step
size to read one or more dimensions at a lower resolution. Similarly,
`stars::write_mdim` can write multidimensional arrays to Zarr
or NetCDF files, or other formats that support the GDAL C++
...
To read parts of a remote (cloud-based) Zarr file, one needs to prepend the
...
* `NA` count values lead to using all values available
* `bounds` variables needed explicit specification because the data
  source did not follow the most recent CF 1.10 conventions; and
