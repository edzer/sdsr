...
gdf = data['gdf']
w_q_B = data['w_q']
w_d183_idw_B = data['w_d183_idw_B']
...
adj_idw = w_d183_idw_B.to_adjlist(remove_symmetric=False)
...
# `cache=TRUE` is deliberately not used here: reticulate's Python
# process is a single persistent session spanning the whole document,
# and knitr's chunk cache only snapshots R-side objects. On a cache
# hit this chunk's Python code would not actually re-run, so `locm_p`
# would never get (re-)assigned in the live Python session, breaking
# every later chunk that references it with a NameError.
...
for ax, col, title in zip(axes[:3], cols, titles):
    # assign to a dummy name (not `_`, see @sec-area) to suppress the
    # per-call Axes/Text echo under the REPL-style python engine used
    # to render this chunk
    _unused = gdf.plot(column=col, ax=ax, cmap=cmap, norm=norm,
                        edgecolor='grey', linewidth=0.01, legend=False)
    _unused = ax.set_title(title)
    _unused = ax.axis('off')

_unused = axes[3].axis('off')

sm_cb = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
_unused = fig.colorbar(sm_cb, ax=axes, orientation='horizontal',
...
for ax, col, title in zip(axes[:3], cols, titles):
    ...
    # per-call Axes/Text echo under the REPL-style python engine used
    # to render this chunk
    _unused = gdf.plot(color=na_color, ax=ax, edgecolor='grey', linewidth=0.01)
    for cat in cat_order:
        subset = gdf[gdf[col] == cat]
        if len(subset) > 0:
            _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
...
_unused = fig.legend(handles=patches, title="Turnout hotspot status\nLocal Moran's I",
...
# Assigning each plot call below to a dummy name (not `_`, see
# @sec-area) suppresses the per-call Axes/Text echo under the
# REPL-style python engine used to render this chunk.
...
for cat in cat_order:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("Local Moran I")
_unused = ax.axis('off')
...
for cat in ['Low', 'High']:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=G_colors[cat], ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("Local Getis/Ord G")
_unused = ax.axis('off')
...
for cat in cat_order:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("Local Geary C")
_unused = ax.axis('off')
...
_unused = fig.legend(handles=all_patches, title="Turnout hotspot status",
...
for cat in cat_order:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("First round turnout\nLocal Geary C")
_unused = ax.axis('off')
...
for cat in cat_order:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("Second round turnout\nLocal Geary C")
_unused = ax.axis('off')
...
for cat in cat_order:
    ...
    if len(subset) > 0:
        _unused = subset.plot(color=cat_colors.get(cat, 'white'), ax=ax,
                               edgecolor='grey', linewidth=0.01)
_unused = ax.set_title("Both rounds turnout\nLocal Multivariate Geary C")
_unused = ax.axis('off')
