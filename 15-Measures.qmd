...
w_q.transform = 'r'
lag_turnout = weights.lag_spatial(w_q, I_turnout.values)
...
```{python}
locm = Moran_Local(I_turnout, w_q, permutations=0)
```
...
```{python}
w_q.transform = 'r'
moran_W = Moran(I_turnout, w_q, permutations=0)
print(locm.Is.sum() / w_q.s0)
print(moran_W.I)
```
...
locm_p = Moran_Local(I_turnout, w_q, permutations=9999, seed=1)
```
...
locm_sad_null = Moran_Local(lm_null.resid, w_q, permutations=9999, seed=1)
...
locm_sad_null_weights = Moran_Local(lm_null_w.resid, w_q, permutations=9999, seed=1)
```
...
locm_sad_types = Moran_Local(lm_types.resid, w_q, permutations=9999, seed=1)
```
::: panel-tabset
#### R
```{r}
...
locm_ex_types = Moran_Local(lm_types.resid, w_q, permutations=99999, seed=1)
```
::: panel-tabset
#### R
```{r}
...
locG = G_Local(I_turnout, w_q, permutations=0)
```
...
```{python}
locG_p = G_Local(I_turnout, w_q, permutations=9999, seed=1)
```
...
locC_p = Geary_Local(connectivity=w_q, permutations=9999, seed=1).fit(I_turnout)
```
...
gdf['hs_C'] = hotspot_C(locC_p, I_turnout, w_q)
...
locC_p_II = Geary_Local(connectivity=w_q, permutations=9999, seed=1).fit(II_turnout)
```
...
```{python}
gdf['hs_C_II'] = hotspot_C(locC_p_II, II_turnout, w_q)
```
...
locMvC_p = Geary_Local_MV(connectivity=w_q, permutations=9999).fit(
...
gdf['hs_MvC'] = hotspot_MvC(locMvC_p, [I_turnout.values, II_turnout.values], w_q)
```
::: panel-tabset
#### R
...
```{python}
nb_card = np.array(w_q.cardinalities.values())
...
gdf['hs_MvCa'] = hotspot_MvCa(locMvC_p, [I_turnout.values, II_turnout.values], w_q)
```
