...
of straight lines in geodetic coordinates (see Exercise 1).
...
as depicted in @fig-antarctica (a) and (c), the
...
@fig-antarctica  shows two different representation of
...
```{r fig-antarctica, out.height = '70%', fig.cap="different representations of Antarctica, (a, c): with a polygon not passing through (-180 -90); (b, d): with a polygon passing through (-180 -90) and (180 -90)", echo=!knitr::is_latex_output()}
#| code-fold: true
# maps:
par(mfrow = c(2,2))
par(mar = c(1,1.2,1,1))
