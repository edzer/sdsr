...
in particular by larger datasets or interactive applications.
...
* using better compression, such as the LERC (Limited Error Raster
...
sub-cubes can be read. Function `stars::read_mdim` has options
for reading sub-cubes by specifying offset and size of the index
in each dimension, and a step size to read one or more dimensions
at a lower resolution. Similarly, `stars::write_mdim` can write
multidimensional arrays to Zarr (or NetCDF) files. Both use the
GDAL C++ multidimensional array API for this.
