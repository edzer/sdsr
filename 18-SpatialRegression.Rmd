...
c(Prediction=unname(coef(OLS_489)[12]), "Unit increment PTRATIO prediction"=mean(nd_fitted - fitted))
```


```{r}
fitted <- predict(SLM_489)
nd_fitted <- predict(SLM_489, newdata=nd_489, listw=lw_q_489, zero.policy=TRUE, legacy=FALSE)
c(Prediction=unname(coef_SLM_489[13]), "Unit increment PTRATIO prediction"=mean(nd_fitted - fitted))
```
```{r}
