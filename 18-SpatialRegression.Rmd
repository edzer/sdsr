...
```{r}
library(lme4)
MLM <- lmer(update(form, . ~ . + (1 | NOX_ID)), data=boston_489, REML=FALSE)
```

```{r}
boston_94$MLM_re <- ranef(MLM)[[1]][,1]
```
...
Delta <- as(model.Matrix(~ -1 + as.factor(NOX_ID), data=boston_489, sparse=TRUE), "dgCMatrix")
M <- as(nb2listw(nb_q_94, style="B"), "CsparseMatrix")
```


```{r}
suppressPackageStartupMessages(library(hglm))
y_hglm <- log(boston_489$median)
X_hglm <- model.matrix(OLS_489)
...
```{r}
boston_94$HGLM_re <- unname(HGLM_iid$ranef)
boston_94$HGLM_ss <- HGLM_sar$ranef[,1]
```


```{r}
library(HSAR)
HSAR <- hsar(form, data=boston_489, W=NULL, M=M, Delta=Delta, burnin=500, Nsim=2500, thinning=1)
```

```{r echo = TRUE, mysize=TRUE, size='\\tiny'}
boston_94$HSAR_ss <- HSAR$Mus[1,]
```
