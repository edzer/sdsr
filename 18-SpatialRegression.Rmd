...
SDEM_489 <- errorsarlm(form, data=boston_489, listw=lw_q_489, Durbin=TRUE, zero.policy=TRUE, control=list(pre_eig=eigs_489))
SEM_489 <- errorsarlm(form, data=boston_489, listw=lw_q_489, zero.policy=TRUE, control=list(pre_eig=eigs_489))
cbind(data.frame(model=c("SEM", "SDEM")), rbind(broom::tidy(Hausman.test(SEM_489)), broom::tidy(Hausman.test(SDEM_489))))[,1:4]
```
...
```{r}
cbind(data.frame(model=c("SEM", "SDEM")), rbind(broom::tidy(LR1.sarlm(SEM_489)), broom::tidy(LR1.sarlm(SDEM_489))))[,c(1, 4:6)]
```
...
SDEM_94 <- errorsarlm(form, data=boston_94, listw=lw_q_94, Durbin=TRUE, control=list(pre_eig=eigs_94))
SEM_94 <- errorsarlm(form, data=boston_94, listw=lw_q_94, control=list(pre_eig=eigs_94))
cbind(data.frame(model=c("SEM", "SDEM")), rbind(broom::tidy(Hausman.test(SEM_94)), broom::tidy(Hausman.test(SDEM_94))))[, 1:4]
```
```{r}
...
SDEM_94w <- errorsarlm(form, data=boston_94, listw=lw_q_94, Durbin=TRUE, weights=units, control=list(pre_eig=eigs_94))
...
res <- rbind(summary(SEM_94)$Coef["I((NOX * 10)^2)",], summary(SEM1_94)$CoefTable["I((NOX * 10)^2)",])
...
SEM1_94iv <- spreg(formiv, data=boston_94, listw=lw_q_94, endog = ~NOX2, instruments=iform, model="error")
...
1/Re(c(RSpectra::eigs(W, k=1, which="SR")$values, RSpectra::eigs(W, k=1, which="LR")$values))
```
...
nd_fitted <- predict(SLM_489, newdata=nd_489, listw=lw_q_489, pred.type="TS", zero.policy=TRUE)
...
suppressWarnings(t1  <- exp(predict(SDEM_489, newdata=nd, listw=lw_q, pred.type="KP2", zero.policy=TRUE)))
suppressWarnings(t2  <- exp(predict(SDEM_489, newdata=nd, listw=lw_q, pred.type="KP5", zero.policy=TRUE)))
data.frame(fit_TS=t0[,1], fit_KP2=c(t1), fit_KP5=c(t2), censored=boston_506$censored[as.integer(attr(t0, "region.id"))])
```
...
boston_94a <- st_cast(aggregate(boston_489[,"NOX_ID"], list(boston_489$NOX_ID), unique), "MULTIPOLYGON")
...
boston_93 <- st_cast(aggregate(boston_487[, "NOX_ID"], list(ids = boston_487$NOX_ID), unique), "MULTIPOLYGON")
...
Delta <- as(model.Matrix(~ -1 + as.factor(NOX_ID), data=boston_487, sparse=TRUE), "dgCMatrix")
...
```{r}
library(HSAR)
suppressWarnings(HSAR <- hsar(form, data=boston_487, W=NULL, M=M, Delta=Delta, burnin=500, Nsim=2500, thinning=1))
```
...
family="gaussian", data=boston_487, method="MCMC", iterations=12000, burnin=2000, 
step=2, seed=123)
```
...
```{r}
library(mgcv)
names(nb_q_93) <- as.character(boston_93$NOX_ID)
boston_487$NOX_ID <- as.factor(boston_487$NOX_ID)
GAM_MRF <- gam(update(form, . ~ . + s(NOX_ID, bs="mrf", xt=list(nb=nb_q_93))), data=boston_487, method="REML")
```

```{r}
boston_93$GAM_ss <- aggregate(predict(GAM_MRF, type="terms", se=FALSE)[,14], list(boston_487$NOX_ID), mean)$x
```


```{r}
res <- rbind(iid_lmer=summary(MLM)$coefficients[6, 1:2], iid_hglm=summary(HGLM_iid)$FixCoefMat[6, 1:2], iid_BX=BX_iid$fixed.effects[6, 1:2], sar_hsar=c(HSAR$Mbetas[1, 6], HSAR$SDbetas[1, 6]), mrf_BX=BX_mrf$fixed.effects[6, 1:2], mrf_GAM=c(summary(GAM_MRF)$p.coeff[6], summary(GAM_MRF)$se[6]))
```

```{r multi-level_coefs, out.width='100%', fig.cap="Polish municipality types 2015", fig=TRUE}
...
```{r multi-level_maps_1, out.width='100%', fig.cap="IID random effects", fig=TRUE}
library(tmap)
...
```{r multi-level_maps_2, out.width='100%', fig.cap="Spatially structured random effects", fig=TRUE}
