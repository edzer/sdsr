...
Here, we'll list the predictions for the censored tract observations using three different precistion types, taking the exponent to get back to the USD median house values.
...
```
```{r}
SDEM_489w <- errorsarlm(form, data=boston_489, listw=lw_q_489, weights=units, Durbin=TRUE, zero.policy=TRUE, control=list(pre_eig=eigs_489))
t0w <- exp(predict(SDEM_489w, newdata=nd, listw=lw_q, pred.type="TS", zero.policy=TRUE))
t0w[,1]
```

```{r}
SEM_t0 <- exp(predict(SEM_489, newdata=nd, listw=lw_q, pred.type="TS", zero.policy=TRUE))
SEM_t0[,1]
```
