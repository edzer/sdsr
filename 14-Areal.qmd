...
# Neither libpysal nor networkx has a built-in sphere-of-influence
# (SOI) graph builder, so this reimplements soi.graph() by hand.
# Following Toussaint (1980) and Avis and Horton [-@avis+horton:1985],
# each point i is given a "sphere of influence": a circle centred on i
# with radius equal to i's own nearest-neighbour distance. A Delaunay
# edge (i, j) is kept only if the two spheres overlap, i.e. if their
# separation does not exceed the SUM of the two radii -- keeping the
# edge when it is within just one point's radius (as if pruning with
# an OR rather than this sum) prunes far too aggressively and yields
# many more disconnected components than R's soi.graph().
kdt = KDTree(coords)
dists, idx = kdt.query(coords, k=2)  # k=2: self + nearest neighbour
nearest = dists[:, 1]
...
for i, neighbors in w_delaunay.neighbors.items():
    for j in neighbors:
        if i < j:
            ...
            if dist_ij <= nearest[i] + nearest[j]:
                soi_neighbors[i].append(j)
                soi_neighbors[j].append(i)

# Represent the pruned graph as a NetworkX graph, the closest
# equivalent to R's "nb" neighbour list object here
G_soi = nx.Graph()
...
# Expect one more component here (17) than R's 16: one Delaunay edge
# sits within a hair's breadth of the sum-of-radii threshold above
# (about 2.5 cm out of ~19 km), so it is kept by R's soi.graph() but
# pruned here, splitting one more pair off the main component. This
# reflects tiny floating-point/centroid differences between R's sf
# and Python's shapely/geopandas, not a remaining logic error.
```
:::
...
for i, nbs in soi_neighbors.items():
    for j in nbs:
        if i < j and (i, j) not in drawn_soi:
            # Assign to a dummy name (not `_`!) to suppress the per-call
            # Line2D echo under the REPL-style python engine used to
            # render this chunk. Assigning to `_` specifically corrupts
            # reticulate's internal state (it uses `_` for its own
            # auto-print bookkeeping), silently breaking print() output
            # in every later Python chunk of the rendered document.
            _unused = ax.plot(
...
# save w_q 
if 'w_q' not in locals():
    w_q = nb_q.to_W()

# Save the necessary variables to a file
with open('ch14_py.pkl', 'wb') as f:
    pickle.dump({'gdf': gdf, 'w_q': w_q, 'w_q_B': w_q_B, 'w_d183_idw_B': w_d183_idw_B}, f)
```
