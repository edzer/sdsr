...
```{r fig-sfobj, out.width = '100%', echo = FALSE}
...
```
as illustrated by @fig-figs2 (left: straight lines on an orthographic projection centered at the plot area).
...
Use of `s2geometry` can be switched of for reasons of performance
...
and dimensions. The dimension table reports the following fields
for each dimension:
...
regular, rotated or sheared grids or other regularly discretized
...
```{r fig-circr, out.width = '70%', echo=!knitr::is_latex_output()}
#| fig.cap: "circular center region of the Landsat 7 scene (band 1)"
#| code-fold: true
...
color breaks, alternatively a numeric sequence of breaks can be
given.  A more familiar view may the RGB or false color composite
shown in @fig-starsrgb.

```{r fig-starsrgb, echo=!knitr::is_latex_output(), message=FALSE}
#| fig.cap: "Two color composites"
#| out.width: 80%
par(mfrow = c(1, 2))
...
`gdal_utils` that provide function arguments identical to the
...
#| fig.height: 5
#| out.width: 80%
#| fig.cap: "Origin destination data zones for Bristol, UK, with zone 33 (E02003043) colored red"
#| code-fold: true
library(spDataLarge)
...
The _tidy data_ paper [@tidy] may suggest that such array data should be processed not as an array, but in a long (unnormalized) table form where each row holds (region, class, year, value), and it is always good to be able to do this. For primary handling and storage however,
...
```{r fig-lineras}
#| fig.cap: "Rasterizing North Carolina county boundaries"
#| out.width: 80%
read_sf(file) |>
