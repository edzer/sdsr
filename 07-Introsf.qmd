...
dimensions (e.g., time), `offset` and `delta` are not `NA`; for
...
```
which gives a data cube with three geometries and six bands.  Aggregation
over a temporal dimension is done by passing a time variable as the
second argument to `aggregate`, as a

* set of time stamps indicating the start of time intervals or
