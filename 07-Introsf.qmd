...
#### Python
```{python}
# geopandas has no direct equivalent of st_is_within_distance()'s
# geodesic (s2-based) distance on ellipsoidal coordinates; reproject
# to a local metric CRS (UTM 17N) first and use planar distances,
# which closely approximates the geodesic result at this regional
# scale (verified against R: same 17 counties)
import geopandas as gpd

orange = nc[nc["NAME"] == "Orange"]
nc_utm = nc.to_crs("EPSG:32617")
orange_utm = orange.to_crs("EPSG:32617").geometry.iloc[0]

within = nc_utm.geometry.distance(orange_utm) <= 50_000
o50 = nc[within.values]
len(o50)
```
:::
...
buf50 = gpd.GeoSeries([orange_utm.buffer(50_000)], crs="EPSG:32617").to_crs(nc.crs)

fig, ax = plt.subplots()
nc.boundary.plot(ax=ax, color="grey", linewidth=0.5)
o50.boundary.plot(ax=ax, color="black", linewidth=1.5)
orange.plot(ax=ax, color="orange")
buf50.boundary.plot(ax=ax, color="brown")
ax.set_axis_off()
plt.show()
```
:::

## Spatial joins {#sec-sf-joins}
...
#### Python
```{python}
#| fig-cap: "Example of a largest-overlap join: the label of the grid cell in the top figure with the largest intersection with a county in the bottom figure is assigned to that county."
#| code-fold: true
# geopandas has no built-in `largest = TRUE` join; it is reproduced
# here with an overlay followed by picking, per county, the
# intersecting grid cell with the greatest overlap area
import geopandas as gpd
import numpy as np
...
nc2264 = gpd.read_file("data/nc.gpkg").to_crs("EPSG:2264")
minx, miny, maxx, maxy = nc2264.total_bounds
n = 10
dx, dy = (maxx - minx) / n, (maxy - miny) / n
letters = "ABCDEFGHIJ"
cells = [box(minx + i * dx, miny + j * dy, minx + (i + 1) * dx, miny + (j + 1) * dy)
         for j in range(n) for i in range(n)]
labels = [f"{letters[9 - j]}{i + 1}" for j in range(n) for i in range(n)]
cmap = plt.get_cmap("tab10")
gr = gpd.GeoDataFrame(
    {"label": labels, "col": [mpl.colors.to_hex(cmap(k % 10)) for k in range(len(cells))]},
    geometry=cells, crs=nc2264.crs,
)
gr = gr.iloc[30:].reset_index(drop=True)  # cut, to verify that NAs work out

inter = gpd.overlay(
    nc2264[["NAME", "geometry"]].reset_index(names="nc_idx"),
    gr.reset_index(names="gr_idx"),
    how="intersection",
)
inter["area"] = inter.geometry.area
best = inter.loc[inter.groupby("nc_idx")["area"].idxmax(), ["nc_idx", "label", "col"]]
nc_j = nc2264.reset_index(names="nc_idx").merge(best, on="nc_idx", how="left")

fig, axes = plt.subplots(2, 1, figsize=(8, 10))
nc_j.boundary.plot(ax=axes[0], color="grey")
gr.plot(ax=axes[0], color=gr["col"], alpha=0.3)
for _, row in gr.iterrows():
    c = row.geometry.centroid
    axes[0].text(c.x, c.y, row["label"], fontsize=7, ha="center")

nc_j.boundary.plot(ax=axes[1], color="grey")
nc_j.plot(ax=axes[1], color=nc_j["col"].fillna("white"))
for _, row in nc_j.dropna(subset=["label"]).iterrows():
    c = row.geometry.centroid
    axes[1].text(c.x, c.y, row["label"], fontsize=6, ha="center")
gr.boundary.plot(ax=axes[1], color="#88ff88", alpha=0.6)
...
### Sampling, gridding, interpolating, dasymetric mapping
...
as explained in @sec-area-weighted, both for intensive and extensive
variables. In addition it can be used for dasymetric mapping of
extensive variables (with an extensive external variable; see
@sec-area-weighted).
...
As an example, we can redistribute the birth counts in the North
Carolina dataset using the density of addresses as a proxy,
disregarding the difference in time of collecting the birth data
(1974-79) and address data (2014).

Address data were downloaded from [NC OneMap](https://www.nconemap.gov/datasets/909e8b7d03e04aecaf7a1afc03376074_0/explore?location=35.173750%2C-79.961050%2C7),
as a GeoJSON file, loaded and converted into address counts for (state boundary clipped) 10 km $\times$ 10 km grid cells by
```{r eval=FALSE}
add.sf = read_sf("data/NC_Master_Address_Dataset_-_2014.geojson")
add.state_plane = st_transform(add.sf, 'EPSG:32119') # NC state plane
nc = st_transform(nc, 'EPSG:32119')
grd = st_make_grid(nc, 10000) |> st_intersection(st_union(st_geometry(nc)))
add.state_plane$ones = 1 # so that sum() gives a count:
grd.addr = aggregate(add.state_plane["ones"], grd, sum)
```
...
grd_addr = gpd.read_file("data/grd_addr.gpkg")
nc = gpd.read_file("data/nc.gpkg").to_crs(grd_addr.crs)
```
:::

Then, dasymetric mapping is used to redistribute county-aggregate
values for `BIR74` and `BIR79` to 10 km cells by
...
#### Python
```{python}
# The Python package tobler provides area_interpolate(), an
# equivalent of st_interpolate_aw() for plain area-weighted
# interpolation, but it has no `weights=` argument: it cannot use
# an auxiliary variable (like the address counts here) to redistribute
# source values *within* each intersection, so it cannot reproduce
# true dasymetric mapping. What follows is therefore plain
# area-weighted interpolation, not dasymetric mapping; the numbers
# will differ from R's (though, as with R, the totals are conserved).
from tobler.area_weighted import area_interpolate

bir_grd = area_interpolate(
    source_df=nc[["BIR74", "BIR79", "geometry"]],
    target_df=grd_addr,
    extensive_variables=["BIR74", "BIR79"],
)
```
:::

We can verify the sums of births at the county level and at the 10 km cell level and see an exact match:
...
#### Python
```{python}
bir_grd[["BIR74", "BIR79"]].sum()
nc[["BIR74", "BIR79"]].sum()
```
:::

At the county-level the comparison can be made by aggregating the cell level values
to counties, using area-weighted interpolation, shown in @fig-aw-scatter.

::: panel-tabset
#### R
```{r fig-aw-scatter, message = FALSE, echo=!knitr::is_latex_output()}
#| fig.cap: "County level birth counts 1974 (x) and 10 km cell values aggregated to county (y); the line is drawn at x = y"
#| code-fold: true
aw = st_interpolate_aw(bir.grd["BIR74"], st_geometry(nc), extensive = TRUE)
plot(nc$BIR74, aw$BIR74, log = 'xy', xlab = 'county-value', 
	 ylab = 'area-weighted interpolated')
abline(0, 1)
```
#### Python
```{python}
...
aw = area_interpolate(source_df=bir_grd[["BIR74", "geometry"]],
                       target_df=nc[["geometry"]],
                       extensive_variables=["BIR74"])

fig, ax = plt.subplots()
ax.scatter(nc["BIR74"], aw["BIR74"])
lims = [1, max(nc["BIR74"].max(), aw["BIR74"].max())]
ax.plot(lims, lims, color="black")
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlabel("county-value")
ax.set_ylabel("area-weighted interpolated")
plt.show()
```
:::
The match at the county level is no longer exact because the values
at the intersection level ($X_{ij}(S_{ij})$ in @sec-area-weighted)
get lost after aggregating to the 10 km cell level.

A map of county-level birth counts is shown in @fig-nc-bir,
the address counts are shown in @fig-addresses and the
dasymetric mapped, redistributed birth counts are shown in Figure
@fig-dasymetric. Birth count ranges differ, but as birth counts are
spatially extensive variables the range depends on the target grid
cell size.

::: panel-tabset
#### R
```{r fig-nc-bir, message = FALSE, echo=!knitr::is_latex_output()}
#| fig.cap: "Birth counts at the county level, 1974"
#| code-fold: true
plot(nc["BIR74"], logz = TRUE, main = NULL)
```
#### Python
```{python}
#| fig.cap: "Birth counts at the county level, 1974"
...
fig, ax = plt.subplots()
nc.plot(column="BIR74", ax=ax, legend=True, norm=mcolors.LogNorm())
ax.set_axis_off()
plt.show()
```
:::
...
::: panel-tabset
#### R
```{r fig-addresses, message = FALSE, echo=!knitr::is_latex_output()}
#| fig.cap: "Number of addresses per 10 km cell, 2014"
#| code-fold: true
grd.addr$ones[grd.addr$ones==0] = 1 # so that logz shows finite values
plot(grd.addr, logz=TRUE, main = NULL) # log scale
```
#### Python
```{python}
#| fig.cap: "Number of addresses per 10 km cell, 2014"
#| code-fold: true
ones = grd_addr["ones"].replace(0, 1)  # so that the log scale shows finite values

fig, ax = plt.subplots()
grd_addr.plot(column=ones, ax=ax, legend=True, norm=mcolors.LogNorm())
ax.set_axis_off()
plt.show()
```
:::
...
::: panel-tabset
#### R
```{r fig-dasymetric, message = FALSE, echo=!knitr::is_latex_output()}
#| fig.cap: "Dasymetric mapping of birth counts, using address counts as external variable"
#| code-fold: true
bir.grd$BIR74[bir.grd$BIR74 == 0] = 1 # so that logz shows finite values
plot(bir.grd["BIR74"], logz = TRUE, main = NULL)
```
#### Python
```{python}
#| fig.cap: "Area-weighted redistribution of birth counts (not dasymetric mapping, see note above)"
#| code-fold: true
vals = bir_grd["BIR74"].replace(0, 1)  # so that the log scale shows finite values

fig, ax = plt.subplots()
bir_grd.plot(column=vals, ax=ax, legend=True, norm=mcolors.LogNorm())
ax.set_axis_off()
plt.show()
```
:::
...
r = rxr.open_rasterio("data/L7_ETMs.tif")
print(r)
```
:::
...
```
#### Python
```{python python-read-tif1}
import rasterio
src = rasterio.open("data/L7_ETMs.tif")
band = src.read(4)
...
```
#### Python
```{python python-riox-tif}
import rioxarray
from shapely.geometry import box, Point
import geopandas as gpd
da = rioxarray.open_rasterio("data/L7_ETMs.tif")
# Write the CRS explicitly if not already set
if da.rio.crs is None:
    da = da.rio.write_crs("EPSG:32724")
...
wkt_points = [
  "POINT (293002.2 9115516)","POINT (290053.1 9110930)",
  "POINT (292911.6 9119069)","POINT (292503.2 9116861)",
  "POINT (291435.7 9113502)","POINT (294899.2 9116963)",
  "POINT (291231.8 9118336)","POINT (298629.5 9112151)",
  "POINT (295431.9 9119798)","POINT (292282 9118130)"  ,
  "POINT (295569.1 9119778)","POINT (295407.6 9117457)",
  "POINT (293100.1 9117561)","POINT (296204.6 9112095)",
  "POINT (297569.3 9110929)","POINT (289053.6 9120451)",
  "POINT (296131.5 9117044)","POINT (297639.8 9110888)",
  "POINT (298543 9116495)",  "POINT (290941.1 9114128)"
]
pts = [wkt.loads(pt_wkt) for pt_wkt in wkt_points]
...
np.random.seed(131)
```
```{python python-xvec-zonal, eval=TRUE}
bbox = da.rio.bounds()
...
trn.loc[[7, 13, 14, 17, 18], "cls"] = "water" # zero-based indexing in Python
...
#### Python
```{python}
# masking with NA requires a floating point array, since
# integer arrays cannot hold NaN
da2 = da.astype("float64").where(da >= 50)
da2
```
:::
...
#### Python
```{python}
daf = da.astype("float64")
ndvi = (daf.sel(band=4) - daf.sel(band=3)) / (daf.sel(band=4) + daf.sel(band=3))
ndvi
```
:::
...
#### Python
```{python}
da.mean(dim=["x", "y"]).to_dataframe(name="mean")
```
:::
...
#### Python
```{python}
# air.rda was exported once from R to data/air_stations.csv
# (station, longitude, latitude) and data/air_long.csv.gz
# (station, date, PM10, in long/tidy form)
import pandas as pd
import geopandas as gpd
import numpy as np
import xarray as xr
...
stations = pd.read_csv("data/air_stations.csv")
long = pd.read_csv("data/air_long.csv.gz", parse_dates=["date"])

wide = long.pivot(index="station", columns="date", values="PM10") \
	.reindex(stations["station"])
geom = gpd.GeoSeries(
	[Point(xy) for xy in zip(stations.longitude, stations.latitude)],
	crs="EPSG:4326"
)

# a vector data cube: "station" is a geometry (point) dimension,
# built using xvec's GeometryIndex
aq = xr.DataArray(
	wide.values, dims=("station", "time"),
	coords={"station": geom.values, "time": wide.columns.values},
	name="PM10",
).xvec.set_geom_indexes("station", crs="EPSG:4326")
aq
```
:::
...
im = ax.imshow(np.log(aq.values), aspect="auto", cmap="viridis")
ax.set_xlabel("time (index)"); ax.set_ylabel("station")
plt.colorbar(im, ax=ax, label="log(PM10)")
plt.show()
```
:::
...
#| fig.cap: "Locations of PM$_{10}$ measurement stations, showing mean values"
#| code-fold: true
de_nuts1 = gpd.read_file("data/de_nuts1.gpkg").to_crs("EPSG:4326")
station_mean = aq.mean(dim="time", skipna=True)
gdf_mean = gpd.GeoDataFrame({"PM10": station_mean.values}, geometry=geom)

fig, ax = plt.subplots(figsize=(6, 6))
de_nuts1.boundary.plot(ax=ax, color="black", linewidth=0.5)
gdf_mean.plot(ax=ax, column="PM10", markersize=25, legend=True, cmap="viridis")
plt.show()
```
:::
...
#### Python
```{python}
# stars' aggregate() spatially joins each point to the polygon it
# falls in, then averages by polygon and time step; xvec has no
# direct equivalent for point-in-polygon data cubes, so we do the
# join with geopandas and average with xarray's groupby
station_pts = gpd.GeoDataFrame(geometry=geom)
region = gpd.sjoin_nearest(
	station_pts, de_nuts1[["NAME_1", "geometry"]]
)["NAME_1"].values

a = aq.assign_coords(region=("station", region)) \
	.groupby("region").mean(dim="station", skipna=True) \
	.reindex(region=de_nuts1["NAME_1"].values) # keep all 16 states
a
```
:::
...
```{python}
#| fig.cap: "Areal mean PM$_{10}$ values, for six days"
sub = a.sel(time=slice("2008-01-01", "2008-01-06"))
de_nuts1_idx = de_nuts1.set_index("NAME_1")

fig, axes = plt.subplots(2, 3, figsize=(10, 8))
for ax, t in zip(axes.ravel(), sub["time"].values):
	gdf_t = de_nuts1_idx.loc[sub["region"].values] \
		.assign(PM10=sub.sel(time=t).values)
	gdf_t.plot(ax=ax, column="PM10", legend=True, cmap="viridis",
			   missing_kwds={"color": "lightgrey"})
	ax.set_title(pd.Timestamp(t).date().isoformat())
...
```{python}
#| fig.cap: "Areal mean PM$_{10}$ time series for a single state"
name = de_nuts1["NAME_1"].iloc[3]  # matches R's de_nuts1$NAME_1[4]
ts = a.sel(region=name).to_series()

fig, ax = plt.subplots(figsize=(9, 3))
ts.plot(ax=ax)
ax.set_title(name); ax.set_ylabel("PM10")
plt.show()
```
:::
...
bristol_zones = gpd.read_file("data/bristol_zones.gpkg")
bristol_od = pd.read_csv("data/bristol_od.csv")

fig, ax = plt.subplots(figsize=(7, 5))
bristol_zones.boundary.plot(ax=ax, color="black", linewidth=0.5)
bristol_zones.iloc[[32]].plot(ax=ax, color="red")  # zone 33, zero-based
plt.show()
```
:::
...
#### Python
```{python}
len(bristol_zones)**2 - len(bristol_od)
```
:::
...
#### Python
```{python}
# pivot_longer() is pandas' melt()
bristol_tidy = bristol_od.drop(columns="all").melt(
    id_vars=["o", "d"], var_name="mode", value_name="n"
)
bristol_tidy.head(6)
```
:::
...
od = bristol_tidy["o"].unique()
mode = bristol_tidy["mode"].unique()
a = np.zeros((len(od), len(od), len(mode)), dtype=int)
a.shape
```
:::
...
#### Python
```{python}
o_idx = {v: i for i, v in enumerate(od)}
mode_idx = {v: i for i, v in enumerate(mode)}
a[bristol_tidy["o"].map(o_idx),
  bristol_tidy["d"].map(o_idx),
  bristol_tidy["mode"].map(mode_idx)] = bristol_tidy["n"].to_numpy()
```
:::
...
#### Python
```{python}
order = [bristol_zones.index[bristol_zones["geo_code"] == z][0] for z in od]
zones = bristol_zones.geometry.iloc[order].reset_index(drop=True)
```
:::
...
#### Python
```{python}
# a plain xarray DataArray needs scalar coordinate labels, so o/d
# could only hold zone *codes*, not the zone geometries themselves --
# unlike a stars vector data cube. The xvec package adds a
# GeometryIndex that lets a dimension coordinate hold actual
# (shapely) geometries, which is what st_dimensions(o = zones, ...)
# does above: it makes odm a genuine vector data cube, letting
# xvec's own plotting and to_geodataframe() methods work spatially
import xarray as xr
import xvec

odm = xr.DataArray(
    a, dims=("o", "d", "mode"),
    coords={"o": zones.values, "d": zones.values, "mode": mode},
    name="N",
).xvec.set_geom_indexes(["o", "d"], crs=bristol_zones.crs)
odm
```
:::
...
#### Python
```{python}
#| fig.cap: "OD matrix sliced for destination zone 33, by transportation mode"
import matplotlib.colors as mcolors

slice33 = odm.isel(d=32) + 1  # zone 33, zero-based; dims (o, mode)
slice33.xvec.plot(col="mode", col_wrap=4, norm=mcolors.LogNorm())
```
:::
...
#### Python
```{python}
d_tot = odm.sum(dim=["o", "mode"])
int(np.argmax(d_tot.values)) + 1  # 1-based, to match R
```
:::
...
```{python}
#| out.height: '90%'
#| fig.cap: "Total commutes, summed by origin (left) or destination (right)"
import pandas as pd

# xarray requires matching dimension *names* to concatenate along a
# new dimension, regardless of index type, so "d" must be renamed to
# "o" first -- this mirrors stars' c(o, d, along = list(od = ...)).
# Renaming also lets GeometryIndex.equals() do a real geometry-aware
# equality check (not just a name check) confirming the "o" and "d"
# zones truly line up before combining them; join="exact" (rather
# than the default "outer") makes concat fail loudly if they don't,
# instead of silently (or, in this early xvec version, buggily)
# realigning by geometry order
d_tot_o = d_tot.rename(d="o")
assert o_tot.xindexes["o"].equals(d_tot_o.xindexes["o"])
x = xr.concat([o_tot, d_tot_o], dim=pd.Index(["origin", "destination"], name="od"),
			  join="exact")
x.xvec.plot(col="od", norm=mcolors.LogNorm())
```
:::
...
```{python}
#| out.height: '90%'
#| fig.cap: "Total commutes per square km, by area of origin (left) or destination (right)"
# division by a plain area array (not itself geometry-indexed) is
# done on the underlying values to avoid xarray trying (and failing)
# to align two different index types on the "o" dimension
area_km2 = zones.to_crs(zones.estimate_utm_crs()).area.to_numpy() / 1e6
o_km = o_tot.copy(data=o_tot.values / area_km2)
d_km = d_tot_o.copy(data=d_tot_o.values / area_km2)

xr.concat([o_km, d_km], dim=pd.Index(["origin", "destination"], name="od"),
		  join="exact") \
  .xvec.plot(norm=mcolors.LogNorm(), col="od")
```
:::
...
This function can also be used to point PROJ to one or more other
resources with datum grids.
