...
raster and vector datacubes (@sec-datacube), supporting
...
software with the tidyverse [@wickham2019welcome], if desired.
...
to an existing layer or replace it; if unset it will error if a
...
e.g. from a database, using `st_delete`.

For file formats supporting a WKT2 coordinate reference system,
...
Use of **s2** can be switched of for reasons of performance
...
did addresses the analysis of time series of vector geometries or raster
grid cells, but did not extend to higher-dimensional arrays.
...
One further field, `values` is hidden as it is not used.  For
regular, rotated or sheared grids or other regularly discretised
dimensions (e.g. time), `offset` and `delta` are not `NA`; for for
...
r[b]
```
selects the circular centre region with a diameter of 500 metre, for
...
reproducing, it should not be set generate further randomly
generated points.)
...
#| fig.cap: "Six 30m Landsat bands downsampled to 90m for Olinda, Br."
#| code-fold: true
plot(r)
```
...
The default colour scale uses grey tones, and stretches these such
that colour breaks correspond to data quantiles over all bands
("histogram equalization"). Setting `breaks = "equal"` gives equal
colour breaks, alternatively a numeric sequence of breaks can be
given.  A more familiar view may the RGB or false colour composite
...
```
A more meaningful function would e.g. compute the NDVI (normalised
...
(e.g. if the sensor discretises the viewing _angle_ in equal sections)
...
`sf::gdal_utils` with function arguments identical to the
...
An air quality data set excerpt from 
the [airBase European air quality data
base](https://www.eea.europa.eu/data-and-maps/data/aqereporting-9),
as it was also used by @RJ-2016-014 (the same data source is used
in @sec-interpolation and @sec-stgeostatistics).
Here, daily average PM$_{10}$ values were computed for rural background
...
### Example: Bristol origin-destination datacube {#sec-oddc}
...
use this to to fill the non-zero entries of `a` using the `bristol_tidy` 
...
```{r fig-lineras}
#| fig.cap: "Rasterising North Carolina county boundaries"
read_sf(file) |>
...
reference system that can be retrieved or replaced with `st_crs`, or be
...
For `stars` object with regular raster dimensions, `st_transform`
will _only_ transform coordinate and always result in a curvilinear
grid. `st_warp` can be used to create a regular raster in a new
coordinate reference system, by regridding (@sec-warp).
...
1. Repeat this after setting `sf_use_s2(FALSE)`, and _compute_ the difference (hint: use `setdiff()`), and colour the counties of the difference using color '#88000088'.
