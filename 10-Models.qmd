...
nc1 = nc.copy()
nc1["SID"] = nc1["SID74"] / nc1["BIR74"]
nc1["NWB"] = nc1["NWBIR74"] / nc1["BIR74"]

model = smf.ols("SID ~ NWB", data=nc1).fit()
pr = model.get_prediction(nc1).summary_frame(alpha=0.05)
pr = pr.rename(columns={"mean": "fit", "obs_ci_lower": "lwr",
						 "obs_ci_upper": "upr"})[["fit", "lwr", "upr"]]
nc1.join(pr).columns.tolist()
```
:::
where we see that 
