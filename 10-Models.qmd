...
would use a spline in two dimensions $s(x,y)$ rather than two independent
splines $s(x)$ and $s(y)$. An exception to this "rule" is when e.g.
a pure latitude effect is desired, for instance to account for
yearly total solar energy influx.
...
@meyerpebesmanc].  Adaptive cross validation measures, e.g. spatial
cross validation may help getting more relevant measures for
