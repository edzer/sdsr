...
applied to spatial data just the way they are applied for predicting
new observations in non-spatial problems:
...
are digitized and points are sampled within these polygons at the
...
for areal data (@sec-area - @sec-spatecon) are deviced for data
...
in color.  For the variable _population count_ one would divide
...
by. It is these totals that inform the (Poisson) variability used
by subsequent modelling, e.g. in CAR-type models (@sec-multilevel).
...
point pattern analysis are considered in their _obervation window_,
...
e.g. caused by cloud coverage, from neihbouring pixels both in
space and time (@gapfill, @heaton).
...
This of course bears a large risk of producing nonsensible results
...
* @sits, who use supervised machine learning and time series deep learning
  to segmentize pixel time series into sequences of land use (time
