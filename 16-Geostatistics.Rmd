...
aq = do.call(cbind, r)
```
We can now select those stations for which we have 75\% of the hourly values measured, i.e. drop those with more than 25\% hourly missing values:
```{r}
...
```
Note that the formula `no2~1` is used to select the variable of interest from the data file (`no2.sf`), and to specify the mean model: `~1` refers to an intercept-only (unknown, constant mean) model.
...
ggplot() + geom_stars(data = s[,,,1:6]) + facet_wrap(~sample) +
...
mean(grd$pop_dens, na.rm = TRUE) * sum(st_area(de)) # verify:
sum(b$Einwohner)
ggplot() + geom_stars(data = grd, aes(fill = sqrt(pop_dens), x = x, y = y))
``
...
```
and subsequently, kriging is done by
```{r}
kr = krige(no2~sqrt(pop_dens), no2.sf, grd["pop_dens"], vr.m)
k$kriging     = k$var1.pred
k$kriging_res = kr$var1.pred
km = merge(k[c("kriging", "kriging_res")])
ggplot() + geom_stars(data = km, aes(fill = X, x = x, y = y)) + 
	geom_sf(data = st_cast(de, "MULTILINESTRING")) + 
	geom_sf(data = no2.sf) + facet_wrap(~X1)
```
where, critically, the `pop_dens` values are now available for
prediction locations in object `grd`.
