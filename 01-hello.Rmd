...
```{r first-map, fig.cap="a first map", eval=TRUE, fig=TRUE, cache=FALSE, message=FALSE}
library(tidyverse)
library(sf)
...
```{r firstgather,fig.cap="ggplot with facet maps", echo=TRUE, cache=FALSE}
...
```{r mapviewfigure, cache=FALSE, echo=TRUE}
...
```{r vectoras,echo=TRUE,fig.cap="The map obtained by rasterizing county total number of births for the period 1974-1979  shown in figure 1.1"}
...
```{r rastertypes01, echo=TRUE, fig.cap="various raster types"}
x = 1:5
y = 1:4
