...
```{r}
(nc.32119 <- nc %>% st_transform(32119))
```
and a single attribute column is selected
```{r}
(nc.32119.bir74 <- nc.32119 %>% select(BIR74))
```
...
these other communities. This is not only important for resolving
problems, but also for reaching consensus on which findings are
facts.
