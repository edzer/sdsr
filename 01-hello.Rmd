...
be discussed in section \@ref(reading). It corresponds to `rgdal::readOGR`, 
but differs in the representation of the returned object. 
The `st_transform`
...
$x$ and $y$ coordinates in a planar system (corresponding to 
`rgdal::spTransform`). It will be discussed
...
```{r gdal-fig,echo=FALSE,fig.cap = "sf and its dependencies; arrows indicate strong dependency, dashed arrows weak dependency"}
...
(corresponding to `rgdal::readGDAL` but differing in the structure of the 
returned object:
```{r}
library(stars)
