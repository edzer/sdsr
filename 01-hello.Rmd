...
```{r first-map, fig.cap="a first map", eval=TRUE, fig=TRUE, cache=FALSE}
library(tidyverse)
library(sf)
...
```{r firstgather,fig.cap="ggplot with facet maps"}
...
plot(x)
```
Similarly, we can write raster data in a `stars` object with `write_stars`
```{r}
