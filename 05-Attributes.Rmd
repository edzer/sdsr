...
e.g. the date of birth of a person or the construction year of
a road. When an attribute such as for instance air quality is a
function of both space and time, time is best handled on equal
footing with geometry (see chapter \@ref(raster)).
...
operations described in the previous chapter (\@ref(opgeom)) operate
...
population counts -- meaning they are not associated with with a
...
e.g. inbetween these two extremes, may also occur.) This chapter
...
literature this is known as _point support_;
...
that are not man-made or associated with a sensor device (such as
remote sensing image pixel boundaries).  

Typical examples for the **aggregate** AGR (block support) variables
are
...
A typical property of such variables is that boundaries come from
legislation, observation devices or analysis choices.
...
* **aggregations**: e.g. sums, totals, averages of fields, counts or densities of objects, associated with lines or regions
...
* lines may be used for objects (roads, rivers), contours of a field, or admininistrative borders
...
```
indicating that the aggregation _function_ (`SUM`) and the
...
respect to geographical coordinate `POINT(-79, 35.5)`) and count
...
```{r ncaggregation, fig.cap="top: SID74 disease cases by county, bottom: summed values by county quadrant, county polygons unioned by county quadrant", echo=!knitr::is_latex_output()}
...
the rectangles of figure \@ref(largest) as the target areas, and
we sum for each rectangle the disease cases of the counties that
_intersect_ with the rectangle, the sum of these will be much larger:
...
Area-weighted interpolation can go either way: interpolate to
larger, smaller or similar sized areas, and may include up- and
downscaling as special cases. Aggregation is typically a matter
of upscaling. 
...
In such cases, statistical approaches could use area-to-point kriging
to estimate values with point support (section \@ref(atpk)). Block
kriging (section \@ref(blockkriging)) estimates block averages from
point values that are not necessarily located within the target
block geometries.

Interesting challenges for downscaling include high-resolution
...
<!--
REFS for both? Second: David Frantz, First: ...
[@sillmann2017understanding]
-->
