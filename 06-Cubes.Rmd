...
dimensions over which we observe, model, or make predictions.
...
Data cubes naturally arise when we observe properties of a set of
geometries repeatedly over time. We define them here as array data
with one or more array dimensions associated with space and/or time
[@lu2018multidimensional], implying that raster data, features with
attributes, and time series data are all special cases of data
cubes. Since we don't restrict to three-dimensionsal structures,
we actually mean _hypercubes_ rather than cubes, and as the cube
size in the different dimensions does not have to be identical,
the better term would be _hyperrectangle_.  For simplicity, we talk
about data cubes instead.
...
thought of as being perpendicular. Arbitrary two-dimensional cube
...
```{r cube, cache = FALSE, fig.cap="Raster data cube with dimensions latitude, longitude and time", echo=!knitr::is_latex_output()}
...
```{r cube4d, cache = TRUE, fig.cap="Four-dimensional raster data cube with dimensions x, y, bands and time", echo=!knitr::is_latex_output()}
...
Figure \@ref(fig:cube4d) shows an example of a four-dimensional data
cube, where three-dimensional data cubes with a spectral dimension
("bands") are organised along a fourth dimension, a time axis. Color
image data always has three bands (R, G, B), this example has
a fourth bands (near infrared, NIR) which is common for remote
sensing data.
...
(x and y) are over within the bands and time dimension.

```{r cube4d2, cache = TRUE, fig.cap="Four-dimensional raster data cube layed out flat over two dimensions", echo=!knitr::is_latex_output()}
# flat ----
...
## Cube dimensions and support

Phenomena in space and time can be thought of as functions
with domain space and time, and with range one or more observed
attributes.  For discrete events or objects, the range is typically
discrete, and precise delineation involves describes the precise
coordinates where it starts or stops, which is best suited by vector
geometries.  For continuous phenomena, there are infinitely many
values to represent and a common approach is to discretize space
and time regularly over the domain extent of interest. This leads
to a number of familiar data structures:
...
times in the future $t'$ at different times $t$. The most general
definition of a data cube is a functional mapping from $n$ dimensions
to $p$ attributes:
...
does this (e.g. a projection). NetCDF files often provide both,
when available.
...
observation or prediction is associated with, but the same notion
applies to temporal support. Although time is rarely reported by
explicit time periods having a start- en end-time, in many cases
either the time stamp implies a period (e.g. "2021" a full year,
"2021-01" a month) or the time period is taken as the period from
the time stamp of the current record up to but not including the
time stamp of the next record.
...
obtained by that. In this figure, the spatial filtering does not
happen by fixing a single spatial dimension at a particular value,
but by selecting a particular subregion, which is a more common
operation. Fixing x or y would give a subcube along a transect of
constant x or y, which can be used to show a Hovm\"{o}ller diagram,
where an attribute is plotted in the space of one space dimension
and time.

```{r cube4filter, cache = TRUE, fig.cap="Data cube filtering by time, band or spatially", echo=!knitr::is_latex_output()}
# filter ----
...
A common analysis case is where one wants to apply a function over
one or more cube dimensions. Simple cases arise where a function
such as `abs` or `sin` is applied to all values in the cube, or
when a function takes all values in the cube and returns a single
scalar, e.g. when one computes the mean or maximum value over the
entire cube. Other options include applying the function to selected
dimensions, e.g. applying a low-pass filter to every individual
(pixel/band) time series (figure \@ref(fig:cube4apply1)) or applying
a _spatial_ low-pass filter to every spatial slice (i.e. every
band/time combination), shown in figure \@ref(fig:cube4apply2).

```{r cube4apply2, cache = TRUE, fig.cap="Low pass filtering of spatial slices", echo=!knitr::is_latex_output()}
# apply ----
...
```{r cube4apply1, cache = TRUE, fig.cap="Low pass filtering of time series", echo=!knitr::is_latex_output()}
...
When applying function mean to an entire datacube, all dimensions
vanish. We can also apply functions to a limited set of dimensions
such that the dimensions vanishes, or is _reduced_. We already saw
filtering as a special case of this, but more in general we could
for instance compute the maximum of every time series, the mean
over every spatial slice, or a band index that summarizes different
spectral values into a single value. Figure \@ref(fig:cube4reduce)
illustrates these options.

```{r cube4reduce, cache = TRUE, fig.cap="Low pass filtering of time series", echo=!knitr::is_latex_output()}
# reduce ----
...
`max`. Grouping geometries can also be `POINT` geometries, in which
case the aggregation function is obsolete as single values at the
`POINT` locations are extracted. In this case, the _two_ spatial
dimensions $x$ and $y$ reduce to a single dimension, the sequence
of feature geometries, which consists of geometries that are defined
in the space of $x$ and $y$.

```{r cube4agg, fig.width=8, cache = TRUE, fig.cap="Low pass filtering of time series", echo=!knitr::is_latex_output()}
# aggregate ----
...
where we could have $PM_{10}$ measurements for

* a set of monitoring stations, and
* a sequence of time intervals

aligned in a vector data cube, or where we consider time series
of demographic or epidemiological data, consisting of (population,
...
## Siwtching dimension with attributes
...
of) $Z_1,Z_2,...,Z_p$. When the $Z_i$ have incompatible measurement
units, one would have to take care when reducing $D_{n+1}$: numeric
functions like `mean` or `max` would be typically meaningless.

Figure \@ref(fig:aqdc) shows a vector data cube for air quality
stations where one cube dimension reflects air quality parameters.

```{r aqdc, fig.width = 7, fig.cap="Vector data cube with air quality time series", echo=!knitr::is_latex_output()}
set.seed(1331)
library(stars)
library(colorspace)
tif = system.file("tif/L7_ETMs.tif", package = "stars")
...
* order of reduction, using `mean`, using `max`, when does it matter?
