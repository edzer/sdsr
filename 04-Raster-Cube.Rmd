...
and we can show the resulting maps for six arbitrarily chosen days
(figure \@ref{fig:airagg}).

```{r airagg, fig.cap="areal mean PM$_{10}$ values, for three days"}
library(tidyverse)
...
geometries for the 102 origin and destination regions, shown in figure \@{fig:bristol1}.
...
array, e.g. for zone 33 (figure \@{fig:bristol1}), by `odm[,,33]`, 
and plot it:
```{r}
plot(odm[,,33])
```
