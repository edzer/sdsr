...
goes at the cost of some data loss and is not reversible.
...
(e.g. if the sensor discretizes the viewing angle in equal parts)
...
the data, e.g. because we want to plot it, is `p * 2` evaluated.
We need data when

* we want to `plot` data
* we want to write an object to disk, with `write_stars`.
* we want to explicitly load an object in memory, with `st_as_stars`

In case the entire object does not fit in memory, `plot` and
`write_stars` each choose a strategy to deal with this:
