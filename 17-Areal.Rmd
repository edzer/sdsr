...
```{r}
library(Matrix)
smat <- as(nb2listw(nb_q, style="B"), "CsparseMatrix")
library(igraph)
g1 <- graph.adjacency(smat)
count_components(g1)
```
...
```{r}
sps <- shortest.paths(g1)
spmax <- apply(sps, 2, max)
max(spmax)
```
...
```{r, echo=FALSE}
pol_pres15$dist_52 <- c(st_distance(coords[mr], coords))/1000
units(pol_pres15$dist_52) <-  NULL
library(ggplot2)
...
(ref:localZvaluesfc) Plots of analytical against conditional permutation standard deviates; left: local Moran's I; right: local G; first round turnout, row-standardised neighbours
