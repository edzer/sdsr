...
```{r moranplot, out.width='100%', fig.cap="Moran plot of I round turnout, row standardised weights", fig=TRUE}
...
```{r moranhat, out.width='100%', fig.cap="Moran plot hat values, row standardised neighbours", fig=TRUE}
...
almost 29\% of the local measures have p-values < 0.05 if no adjustment is applied, but only 12\% using Bonferroni correction.
...
```
This approach is not provided as a function in **spdep**. The outcome is that almost 32\% of observations have two sided p-values < 0.05 without multiple comparison correction, and under 3\% with Bonferroni correction. Since it is not advisable to use conditional permutation with regression residuals in the presence of spatial autocorrelation, `localmoran.sad()` and `localmoran.exact()` provide wys of calculating more robust standard deviates with the added flexibility of a possibly richer mean model.
...
```{r localmoranZi, out.width='100%', fig.cap="Local Moran's I standard deviates by method", fig=TRUE}
...
```{r localmoranZimap, out.width='100%', fig.cap="Local Moran's I standard deviates", fig=TRUE}
...
```{r localGimap, out.width='100%', fig.cap="Local G standard deviates", fig=TRUE}
...
```{r localHimap, out.width='100%', fig.cap="Local spatially weighted mean values", fig=TRUE}
...
```{r EVmap, out.width='100%', fig.cap="First four eigenvectors", fig=TRUE}
