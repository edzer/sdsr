...
Measures of spatial autcorrelation unfortunately pick up other mis-specifications in the way that we model data [@WallerGotway:2004; @McMillen:2003]. For reference, Moran's $I$ is given as [@cliff+ord:81, page 17]:
...
```
Geary's global $C$ is implemented in `geary.test()` largely following the same argument structure as `moran.test()`. Geary's global $C$ measure is given as [@cliff+ord:81, page 17]:

$$
C = \left( \frac{(n-1)}{2 S_0} \right) \frac{\sum_{(2)} w_{ij}(x_i-x_j)^2}{\sum_{i=1}^{n} z_i^2}
$$
using the same definitions as for Moran's $I$.
...
```
The Getis-Ord $G$ test includes extra arguments to accommodate differences between implementations, as Bivand and Wong [-@Bivand2018] found multiple divergences from the original definitions, often to omit no-neighbour observations generated when using distance band neighbours. It is given as [@getis+ord:92, page 194]:

$$
G = \frac{\sum_{(2)} w_{ij} x_i x_j}{\sum_{(2)} x_i x_j}
$$
using the same definitions as for Moran's $I$. For $G_*$, the $\sum_{(2)}$ constraint is relaxed by including $i$ as a neighbour of itself (thereby also removing the no-neighbour problem, because all observations have at least one neighbour).
