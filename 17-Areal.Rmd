...
```{r Iihotspots, out.width='100%', fig.cap="Local Moran's I hotspot maps $\alpha = 0.05$: left panel: unadjusted analytical standard deviates; right panel: Bonferroni adjusted conditional permutation standard deviates, first round turnout, row-standardised neighbours", fig=TRUE}
...
```{r local_Z_values, out.width='100%', fig.cap="Plots of analytical against conditional permutation standard deviates; left: local Moran's I; right: local G; first round turnout, row-standardised neighbours", fig=TRUE}
library(ggplot2)
...
summary(Geoda_w)
system.time(lisa <- local_moran(Geoda_w, pol_pres15["I_turnout"], cpu_threads=ifelse(detectCores() == 1, 1, detectCores()-1L), permutations=499, seed=1))
