...
Here the size of `k=` is sufficient to ensure connectedness, although the graph is not planar as edges cross at locations other than nodes:
```{r}
...
unlist(spweights.constants(lw_q_W))
```
...
unlist(spweights.constants(lw_q_S))
```

```{r}
lw_d183_B <- nb2listw(nb_d183, style="B")
unlist(spweights.constants(lw_d183_B))
```

```{r}
gwts <- lapply(nbdists(nb_d183, coords), function(x) 1/(x/250))
lw_d183_idw_B <- nb2listw(nb_d183, glist=gwts, style="B")
unlist(spweights.constants(lw_d183_idw_B))
```
...
unlist(spweights.constants(lw_d16_B,  zero.policy=TRUE))
```

```{r}
unlist(spweights.constants(lw_d16_B,  zero.policy=TRUE, adjust.n=FALSE))
```
