...
```
The Getis-Ord test includes extra arguments to accommodate differences between implementations
...
```{r}
mamc <- sp.mantel.mc(pol_pres15$I_turnout, listw=lw_q_B, nsim=999, type="sokal", return_boot = TRUE)
if (broom_ok) broom::tidy(mamc) else glance_boot(mamc)
```


```{r}
leemc <- lee.mc(pol_pres15$I_turnout, pol_pres15$II_turnout, listw=lw_q_B, nsim=999, return_boot = TRUE)
if (broom_ok) broom::tidy(leemc) else glance_boot(leemc)
```
...
```{r moran_plot, out.width='100%', fig.cap="Moran plot of I round turnout, row standardised neighbours", fig=TRUE}
infl_W <- moran.plot(pol_pres15$I_turnout, listw=lw_q_W, labels=as.character(pol_pres15$TERYT), cex=1, pch=".", xlab="I round turnout", ylab="lagged turnout")
```
