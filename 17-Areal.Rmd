...
Measures of spatial autocorrelation unfortunately pick up other mis-specifications in the way that we model data [@WallerGotway:2004; @McMillen:2003]. For reference, Moran's $I$ is given as [@cliff+ord:81, page 17]:
...
```{r localmoranZ, out.width='100%', fig.cap="Analytical and conditional permutation standard deviates of local Moran's I for first round turnout, row-standardised neighbours", fig=TRUE}
...
```
The figure shows that conditional permutation scales back the proportion of standard deviate values taking high values, especially positive values. As we will see below, the analytical standard deviates of local Moran's $I$ should probably not be used if alternatives are available.
...
```{r localZvalues, out.width='100%', fig.cap="Plots of analytical against conditional permutation standard deviates; left: local Moran's I; right: local G; first round turnout, row-standardised neighbours"}
library(ggplot2)
...
```{r localG, out.width='100%', fig.cap="Left: analytical standard deviates of local G; right: Bonferroni hotspots; first round turnout, row-standardised neighbours", fig=TRUE}
...
In this case, we do not need to contrast the two estimation methods, and showing the mapped standard deviate is as informative as the "hotspot" status for the chosen adjustment. In the case of $G_i$, the values taken by the measure reflect the values of the input variable, so a "High cluster" is found for observations with high values of the input variable, here high turnout in metropolitan areas.
