...
```{r}
locm_nml <- localmoran(pol_pres15$I_turnout, listw=lw_q_B, alternative="two.sided", mlvar=FALSE)
```
...
library(ggplot2)
ggplot(df) + geom_density(aes(x=std_deviate, fill=method), alpha=0.2) + xlim(c(-6, 12))
```
...
```{r}
print(do.call("rbind", tapply(df$std_deviate, list(df$method), summary)), digits=3)
```
...
```{r}
z_sim_1 <- (loc_G_ps$Gs - apply(loc_G_ps$rGs, 1, mean))/apply(loc_G_ps$rGs, 1, sd)
