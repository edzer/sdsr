...
do.call("rbind", lapply(jcl, function(t) {
  x <- matrix(c(t$estimate, unname(t$statistic)), nrow=1)
  colnames(x) <- c("Joincount", names(t$estimate)[2:3], "z-value")
  nm <- substring(names(t$statistic), 18)
  rownames(x) <- paste0(nm, ":", nm)
  x
}))
```
...
```{r}
moran.test(pol_pres15$I_turnout, listw=lw_q_B, randomisation=FALSE)
```
...
```{r, cache=TRUE}
lm.morantest.exact(ols, listw=lw_q_B)
```
...
```{r}
geary.test(pol_pres15$I_turnout, listw=lw_q_B)
```
...
```
```{r}
globalG.test(pol_pres15$I_turnout, listw=lw_q_B)
```
...
```{r}
moran.mc(pol_pres15$I_turnout, listw=lw_q_B, nsim=999, return_boot = TRUE)
```
...
```{r}
sp.mantel.mc(pol_pres15$I_turnout, listw=lw_q_B, nsim=999, type="sokal", return_boot = TRUE)
```
