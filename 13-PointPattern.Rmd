...
```{r pp1, fig.height=6, fig.cap = "Depending on the observation window (grey), the same point pattern can appear completely spatially random (left), or clustered (right)", echo=!knitr::is_latex_output()}
...
\@ref(fig:quadrat))
```{r quadrat, fig.height=6, fig.cap = "3 x 3 quadrat counts for the two point patterns", echo = !knitr::is_latex_output()}
...
```
```{r bwdiggle, fig.height = 6, fig.cap = "Kernel densities for both point patterns", echo=!knitr::is_latex_output()}
...
Another spatial random sampling type supported by `sf` natively (in
`st_sample`) is random points on the sphere. An example of this is
shown in figure \@ref(fig:srsglobe), where points were constrained
to those in oceans.
