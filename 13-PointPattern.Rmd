...
```{r pp1, fig.height = "70%", fig.cap = "Depending on the observation window (grey), the same point pattern can appear completely spatially random (left), or clustered (right)", echo=!knitr::is_latex_output()}
...
\@ref(fig:quadrat))
```{r quadrat, fig.height="60%", fig.cap = "3 x 3 quadrat counts for the two point patterns", echo = !knitr::is_latex_output()}
...
```
```{r bwdiggle, fig.height = "70%", fig.cap = "Kernel densities for both point patterns", echo=!knitr::is_latex_output()}
...
plot(m, se = FALSE)
```
The model also has a `predict` method, invoked by
```{r}
p = predict(m, covariates = list(dist = as.im(s2["dist"])))
```
which can be converted back using `st_as_stars`, and plotted (figure \@ref(fig:ppmpred)).
```{r ppmpred, fig.cap = "Predictions as stars object", echo=!knitr::is_latex_output()}
plot(st_as_stars(p), breaks = "equal")
```
...
the coordinate reference system we started with.
...
nrow(th)
```
```{r rThomas, fig.cap = "Thomas process with mu = 3 and scale = 0.05", echo=!knitr::is_latex_output()}
plot(w2)
plot(th, add = TRUE)
```
...
size obtained within the observation window is a random variable.

## Simulating points on the globe

Another spatial random sampling method supported by `sf` natively
(and not by `spatstat`) is random points on the sphere. An example
of this is shown in figure \@ref(fig:srsglobe).

```{r srsglobe, fig.cap = "Randomly distributed points over the oceans", echo=!knitr::is_latex_output()}
