...
```{r}
nc_db %>% filter(ST_Intersects(wkb_geometry, 'SRID=4267;POINT (-81.49826 36.4314)'))
nc_db %>% filter(ST_Area(wkb_geometry) > 0.1) %>% head(3)
```

It should be noted that PostGIS' `ST_Area` computes the same area
as the `AREA` field in `nc`, which is the meaningless value obtained
by assuming the coordinates are Cartesian, which they are not.
...
```{r,eval=FALSE}
download.file("https://openstreetmap.org/api/0.6/map?bbox=7.595,51.969,7.598,51.970",
			  "data/ms.osm")
```
...
bb = st_bbox(c(xmin=7.595, ymin = 51.969, xmax = 7.598, ymax = 51.970),
	crs = 4326)
plot(st_as_sfc(bb), axes = TRUE, lwd = 2, lty = 2)
plot(o[,1], lwd = 2, add = TRUE)
plot(st_geometry(p), border = NA, col = '#88888888', add = TRUE)
```
The overpass API provides a much more generic and powerful query
...
zones). An R package that can do that is gdalcubes [@R-gdalcubes; @appel2019demand].
