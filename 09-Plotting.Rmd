...
plot(st_geometry(w))
```
and we see that this is the default projection for data with geographic coordinates, as indicated by
```{r}
st_crs(w)
```
...
data in mind, and work exclusively with geographic coordinates.
