...
```{r tmapnc, fig.cap = '(ref:tmap)', echo = knitr::is_latex_output()}
library(tmap)
...
```
```{r tmapnc2, fig.cap = '(ref:tmapfacets)', echo = FALSE}
nc.32119 %>% select(SID74, SID79) %>% 
	pivot_longer(starts_with("SID"), values_to = "SID") -> nc_longer
tm_shape(nc_longer) + tm_polygons("SID") + tm_facets(by = "name")
```
...
```{r tmapstars, fig.cap = "Simple raster plot with tmap", echo=FALSE}
tm_shape(r) + tm_raster()
```
