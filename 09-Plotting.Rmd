...
default for `ggplot2::geom_sf()` (section \@ref(geomsf)).
...
```{r figreset, out.height = '80%', fig.cap = "Annotating base plots that have a legend"}
library(sf)
...
shown, as in
```{r starshook, fig.cap = "annotated multi-slice stars plot", message=FALSE}
library(stars)
...
(ref:tmap) tmap: using `tm_polygons()` with two attribute names

```{r tmapnc, out.width = '80%', fig.cap = '(ref:tmap)', echo = knitr::is_latex_output()}
library(tmap)
...
(ref:tmapfacets) tmap: using `tm_facets()` on a long table
 
to recreate figure \@ref(fig:tmapnc) and
```{r eval=FALSE}
nc.32119 %>% select(SID74, SID79) %>% 
	pivot_longer(starts_with("SID"), values_to = "SID") -> nc_longer
tm_shape(nc_longer) + tm_polygons("SID") + tm_facets(by = "name")
```
```{r tmapnc2, out.width = '80%', fig.cap = '(ref:tmapfacets)', echo = FALSE}
nc.32119 %>% select(SID74, SID79) %>% 
	pivot_longer(starts_with("SID"), values_to = "SID") -> nc_longer
tm_shape(nc_longer) + tm_polygons("SID") + tm_facets(by = "name")
```
...
```{r tmapstars, out.width = '80%', fig.cap = "simple raster plot with tmap", echo=FALSE}
tm_shape(r) + tm_raster()
```
...
1. Recreate the plot in figure \@ref(fig:tmapstars) using the `viridis` colour ramp.
