...
but otherwise it is entirely independent from, and incompetible
...
we use
```{r tmapnc tmapnc2,fig.cap = "tmap: using tm_polygons with two attributes"}
tm_shape(nc.32119) + tm_polygons(c("SID74", "SID79"))
```
to create figure \@ref(fig:tmapnc) and
```{r tmapnc2,fig.cap = "tmap: using tm_facets"}
...
```
is shown in figure \@ref(fig:tmapstars).
