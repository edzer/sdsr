...
equal footing with geometry (e.g. in a data cube, see 
@sec-datacube).
...
associated with either nodes (points), edges (lines) or faces
(areas, cells) for instance when they were used to approximate
partial differential equations [@Haltiner, @Collins13]
...
have many duplicate boundaries, and hence not be _valid_
(@sec-valid).

```{r fig-ncaggregation, out.width = '70%', fig.cap="SID74 counts by county quadrant, with county polygons unioned by county quadrant", echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-ncblocks, out.width = '70%', fig.cap="example target blocks layed out over North Carolina counties", echo=!knitr::is_latex_output()}
#| code-fold: true
...
```{r fig-awiex, out.width = '50%', fig.cap = "example data for area-weighted interpolation", echo = !knitr::is_latex_output()}
#| code-fold: true
