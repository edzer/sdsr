...
```
```{r setup_sr1, echo=FALSE, include=FALSE}
...
# pyinla (see @sec-spatglmm) fits the IID and besag (ICAR) random
# effects used earlier in the book, but its current model catalogue
# has no counterpart for INLA's "slm" latent model: a specialised,
# still-experimental spatial-lag prior on the response itself, built
# from `rho.min`/`rho.max`/`W` rather than a simple id + graph
# specification. There is therefore no `SDEM_506_slm` equivalent here.
```
:::
