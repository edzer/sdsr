...
```
where we see that udunits and GDAL units are integrated. The axis
lengths (`SemiMajor`, `SemiMinor`) and/or the (inverse) flattening
are used e.g. to compute great circle distances on the ellipsoid,
using the algorithm from @karney2013algorithms, which is implemented
in PROJ, and interfaced by `lwgeom::st_geod_distance`, which in
turn is called by `sf::st_distance` in case an object has geodetic
coordinates.
