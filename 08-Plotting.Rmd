...
How we plot lines, symbols, polygons (choroplets; overlapping
polygons), rasters
...
```{r eqc,fig.cap="Germany in equidistant cylindrical projection: left with degrees, right with metres along the axes"}
par(mfrow = c(1, 2))
plot(DE, axes = TRUE)
...
No. Look for instance at the figure \@ref(fig:eqc), left. Countries
...
```
How long is this linestring? Let's see:
```{r}
st_length(ls)
```
Pfew. But does `ls` actually intersect with the dateline?
```{r}
dateline = st_sfc(st_linestring(rbind(c(180, 51), c(180, 53))), crs = 4326)
st_intersects(ls, dateline)
```
... it seems not!? How can this be? The warning sayd it all: if `ls`
is not spherical, it means it follows the 52th parallel, crossing
`(0, 52)` half way. As if we drew a straight line between the two
points on the left figure of \@ref(fig:world).

Why all these inconsistencies? The software `sf` is built upon
(see the C/C++ libraries box in figure \@ref(fig:gdal-fig)) is a
GIS stack, which originally was (and still is!) dominantly about
flat, 2D geometries. The simple feature standard assumes straight
lines between points, but great circle segments are not straight.
The functions that deal with spherical geometry, such as `st_length`,
use PostGIS extensions in liblwgeom that were added later on, without
rewriting the entire core for geodetic coordinates.  More recent
systems, including as Google's S2^[http://s2geometry.io/], BigQuery
GIS^[https://cloud.google.com/bigquery/docs/gis-intro] and Uebers
H3^[https://eng.uber.com/h3/] were written with global data in mind
from scratch, and only work with geodetic coordinates.
...
dateline. Also note that bounding boxes
```{r}
st_bbox(ls.w)
```
simply take the coordinate ranges, and are pretty much meaningless
for geometries that cross the dateline. 
