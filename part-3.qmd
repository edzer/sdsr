...
amount of concepts that we could safely classify as "models for
...
* simple feature geometries (@sec-geometries), how straight lines
  between points can be used to define linestrings and polygons
* the set of geometry types explained in @sec-seven
* support and the way feature attributes relate to geometries
(@sec-featureattributes)
* how simple tesselations can describe space-time volumes
(@sec-datacube)
* how these concepts can be made operational using data science
  software (@sec-sf)
...
traditionally modelled as

$$\mbox{observed}=\mbox{explained}+\mbox{error}$$

where "error" does not refer to mistakes, but to variation that is
not explained, including measurement error but also lack of fit or
variation caused by model misspecification. For spatial data, a
further term is often helpful, as in

$$\mbox{observed}=\mbox{explained} + \mbox{smooth} + \mbox{rough}$$

where "smooth" refers to variation that is not explained (e.g.
by external predictors) but that clearly shows "smooth" spatial
patterns, as opposed to the "rough" residual which does not do this.
Such a "smooth" term can for instance be modelled by base functions
...
focus on the three "classical" spatial statistics topics: analysis
