...
where we think we "do nothing" (@fig-world), left ,
or where we show the world "as it is", e.g. as seen from space
(@fig-world, right).

```{r fig-world, fig = TRUE, echo = !knitr::is_latex_output(), message = FALSE}
#| fig.cap: "Earth country boundaries; left: mapping long/lat linearly to $x$ and $y$ (plate carrée); right: as seen from infinite distance (orthographic)"
library(sf)
...
The projection taken in @fig-world) (left  is the
...
default for `ggplot2::geom_sf()` (@sec-geomsf).
...
```
When we now plot both maps (@fig-eqc) , they look
identical up to the values along the axes: degrees for ellipsoidal
(left), and metres for projected (Cartesian) coordinates.

```{r fig-eqc,out.width='60%', fig.cap="Germany in equirectangular projection: with axis units degrees (left) and metres in the equidistant cylindrical projection (right)"}
par(mfrow = c(1, 2))
plot(DE, axes = TRUE)
...
* colours ramps often stretch non-linearly over the value range,
...
### Color breaks: `classInt` {#sec-classintervals}
...
A graticule is a network of lines on a map that follow constant latitude or
longitude. On @fig-first-map  a graticule is drawn
in grey, on @fig-firstgather  in white. 
Graticules are often drawn in maps to give reference where
something is. In our first map in @fig-first-map  we can
...
The real navigation aid on @fig-world  and most other
...
in two. If one wants to _add_ to an existing plot having a coulor
...
```{r fig-figreset, out.height = '80%', fig.cap = "Annotating base plots that have a legend"}
library(sf)
...
shown, as in
```{r fig-starshook, fig.cap = "annotated multi-slice stars plot", message=FALSE}
library(stars)
...
overplotting, this is not being checked.

### Colors and coulor breaks

In base plots, `nbreaks` can be used to set the number of coulor
...
for instance figure
@fig-ggplotstars is created by the following commands:
```{r fig-ggplotstars,fig.cap="Simple raster plot with ggplot2"}
library(ggplot2)
library(stars)
r <- read_stars(system.file("tif/L7_ETMs.tif", package = "stars"))
...
chosen. To recreate @fig-firstgather , for instance,
we use


```{r fig-tmapnc, out.width = '80%', echo = knitr::is_latex_output()}
...
```
```{r fig-tmapnc2, out.width = '80%', echo = FALSE}
...
```{r fig-tmapstars, out.width = '80%', fig.cap = "simple raster plot with tmap", echo=FALSE}
tm_shape(r) + tm_raster()
```
is shown in @fig-tmapstars. More examples of the use of **tmap** are given in @sec-area.
