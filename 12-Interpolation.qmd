...
in @fig-plotDE.
<!---
Maybe reconfigure air in "spacetime" - sp gets loaded and attached because
DE_NUTS1 is "SpatialPolygonsDataFrame", with anachronistic output; GPKG?
--->
```{r plot}
...
```
shown by the drawn line in @fig-fitvariogrammodel.
...
parameter estimation [e.g., @kitanidis1985maximum], which for
...
equal to the number of observations; @heaton compares approaches
...
```
We see that the signal is similar, but that the simple 
<!---
should simple be sample?
--->
means are
...
value: 
<!---
would set.seed() be effective (which RNG is used internally); I see 
equality in runs when set.seed(same_seed) is used before each run?
--->
