...
e.g. @DiggleTawnMoyeed:98, @Diggle:2007, @blangiardo2015spatial,
and @wikle2019spatio. An overview and comparisons of methods for
large datasets is given in @Heaton2018.
...
create a regular grid with 10 km grid cells (pixels) over Germany by
...
```
shown in @fig-fitvariogrammodel.
```{r fig-fitvariogrammodel, echo = !knitr::is_latex_output()}
#| fig.cap: "Sample variogram (circles) with models fitted using weighted least squares (drawn line) and maximum likelihood estimation (dashed line)"
#| code-fold: true
...
par(xaxs = "i", yaxs = "i")
plot(gamma ~ dist, v, xlim = c(0, 1.075 * max(v$dist)), ylim = c(0, 1.05 * max(v$gamma)),
   xlab = "distance [m]", ylab = expression(gamma(h)))
...
minimizing $\sum_{i=1}^{n}w_i(\gamma(h_i)-\hat{\gamma}(h_i))^2$,
...
As an alternative to weighted least squares fitting one can use
maximum likelihood (ML) or restricted maximum likelihood parameter
estimation [e.g., @kitanidis1985maximum], which for this case leads
to a relatively similar fitted model, shown as the dashed line in
@fig-fitvariogrammodel. An advantage of ML-type approaches is that
it does not require choosing distance intervals $h_i$ and weights
$w_i$. Disadvantages are that it leans on stronger assumptions of
multivariate normally distributed data, and for larger datasets
that it requires iteratively solving linear systems of size equal
...
we see that the simple approach gives more variability and mostly
...
neigbours to include in kriging estimation by setting `nmax`
...
sum(b$Einwohner)
```
which indicates we did not loose many: using `st_interpolate_aw` should
