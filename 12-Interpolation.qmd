...
We can read NO$_2$ data, which is prepared in @sec-stgeostatistics, from
package `gstat` using
```{r, message=FALSE}
library(tidyverse)
...
```{r fig-vgm, fig.cap = "Sample variogram plot",echo=!knitr::is_latex_output()}
...
```
shown in @fig-vgm2 .
```{r fig-vgm2,fig.cap = "Sample variogram plot with adjusted cutoff and lag width"}
...
```{r fig-fitvariogrammodel, fig.cap = "Sample variogram with fitted model", echo = !knitr::is_latex_output()}
...
```{r fig-no2scat, out.width = '70%', eval=TRUE, fig=TRUE, echo=!knitr::is_latex_output()}
