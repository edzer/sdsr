...
binary predicate_ (`sgbp`) object, is returned by all binary predicate functions (which are shown below). `sgbp` objects are lists of indices of the `TRUE` values in each row:
```{r}
...
extended 9-intersection model (DE-9IM) is adopted, which is explained in more detail on
[Wikipedia](https://en.wikipedia.org/wiki/DE-9IM) [@de9im1,
@de9im2].  Briefly, DE-9IM 
...
When working on geographic coordinates (degrees longitude/latitude), package `sf`
