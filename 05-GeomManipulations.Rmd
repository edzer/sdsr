...
```
With a precision larger than 1, the number of decimal placed to round
to are specified; for 1000, this is 3:
```{r}
p %>% st_set_precision(1000) %>% st_as_binary() %>% st_as_sfc() %>% `[[`(1)
```
With a precision between 0 and 1, rounding to the nearest `1/precision`
is obtained:
```{r}
...
computations.  Which precision values are best for which
application is often a matter of trial and error. A
reproducible example illustrating precision is found
