...
would have led to the error message
```
Error in CPL_nary_intersection(x) :
  Evaluation error: TopologyException: found non-noded intersection between LINESTRING (0.329035 -0.0846201, 0.333671 -0.0835073) and LINESTRING (0.330465 -0.0842769, 0.328225 -0.0848146) at 0.32965918719530368 -0.084470389572422672.
Calls: st_intersection ... st_intersection -> st_intersection.sfc -> CPL_nary_intersection
```
but in addition decreasing `buf_size` to 0.7 would also not have generated
an error.

```{r invalid, fig.cap="n-ary intersection that may lead to invalid geometries", echo=TRUE, eval=TRUE, fig=TRUE}
