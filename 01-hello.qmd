...
gdf_geo = gpd.read_file("data/nc.gpkg")  # native NAD27 geographic CRS
gdf = gdf_geo.to_crs("EPSG:32119")

# build a graticule by reprojecting meridians/parallels from geographic
# coordinates into the plot CRS, mirroring sf's plot(graticule = TRUE)
minx, miny, maxx, maxy = gdf_geo.total_bounds
lon_ticks = np.arange(np.ceil(minx), np.floor(maxx) + 1)
lat_ticks = np.arange(np.ceil(miny), np.floor(maxy) + 1)
to_plot_crs = Transformer.from_crs(gdf_geo.crs, gdf.crs, always_xy=True)

lines = []
ys = np.linspace(miny, maxy, 100)
for lon in lon_ticks:
    lines.append(LineString(zip(*to_plot_crs.transform(np.full_like(ys, lon), ys))))
xs = np.linspace(minx, maxx, 100)
for lat in lat_ticks:
    lines.append(LineString(zip(*to_plot_crs.transform(xs, np.full_like(xs, lat)))))
graticule = gpd.GeoSeries(lines, crs=gdf.crs)

fig, ax = plt.subplots(1, 1)
graticule.plot(ax=ax, color="grey", linewidth=0.5)
gdf.plot(column="BIR74", ax=ax, legend=True,
         legend_kwds={"orientation": "horizontal"})

# label the frame with degree values, as sf's graticule axes do
midlat, midlon = (miny + maxy) / 2, (minx + maxx) / 2
xt, _ = to_plot_crs.transform(lon_ticks, np.full_like(lon_ticks, midlat))
_, yt = to_plot_crs.transform(np.full_like(lat_ticks, midlon), lat_ticks)
ax.set_xticks(xt)
ax.set_xticklabels([f"{-v:.0f}°W" for v in lon_ticks])
ax.set_yticks(yt)
ax.set_yticklabels([f"{v:.0f}°N" for v in lat_ticks])
ax.set_title("BIR74")
plt.show()
```
...
gdf = gpd.read_file("data/nc.gpkg")
plt.style.use('ggplot')

# share one colour scale across both facets, matching ggplot's single
# scale_fill_gradientn() for both panels
vmin = min(gdf["SID74"].min(), gdf["SID79"].min())
vmax = max(gdf["SID74"].max(), gdf["SID79"].max())

fig, axes = plt.subplots(nrows=2, figsize=(10, 7))
gdf.plot(column="SID74", ax=axes[0], vmin=vmin, vmax=vmax, legend=True)
gdf.plot(column="SID79", ax=axes[1], vmin=vmin, vmax=vmax, legend=True)
...
#| code-fold: true
#| output: asis
# reticulate has no automatic bridge from a Python object's
# _repr_html_() to knitr's rich HTML output (unlike R htmlwidgets,
# which mapview uses and which knitr renders automatically), so the
# raw HTML needs to be printed explicitly, with this chunk marked
# `output: asis` so it is inserted directly rather than as printed text
m = gdf.explore("BIR74", legend = True)
print(m._repr_html_())
```
:::
...
show(band1, transform=src.transform, ax=axes[0, 0])
axes[0, 0].axis(False)
axes[0, 0].set_title("(a)")

b1_10x10 = band1[0:10, 0:10]
axes[0, 1].imshow(b1_10x10)
axes[0, 1].axis(False)
for (j, i), label in np.ndenumerate(b1_10x10):
    axes[0, 1].text(i, j, label, ha="center", va="center", fontsize=6)
axes[0, 1].set_title("(b)")

# sample 3 random points within the raster's true spatial extent,
# matching st_sample(st_as_sfc(st_bbox(x)), 3)
rng = np.random.default_rng(131)
pts = gpd.GeoSeries(
    [Point(x, y) for x, y in
     zip(rng.uniform(minx, maxx, 3), rng.uniform(miny, maxy, 3))],
    crs=src.crs,
)

show(band1, transform=src.transform, ax=axes[1, 0])
pts.plot(ax=axes[1, 0], marker="+", color="blue")
axes[1, 0].axis(False)
axes[1, 0].set_title("(c)")

show(band1, transform=src.transform, ax=axes[1, 1])
pts.buffer(500).plot(ax=axes[1, 1], facecolor="none", edgecolor="blue", linewidth=2)
...
# query at points: rasterio.sample returns all bands for each point
coords = [(p.x, p.y) for p in pts]
list(src.sample(coords))
```
```{python}
#| code-fold: true
# aggregate (mean) over circles: mask each buffer polygon and average
# the pixels it covers
buffers = pts.buffer(500)
means = []
for geom in buffers:
    out_image, _ = mask(src, [geom], crop=True, nodata=src.nodata)
    band = out_image[0].astype(float)
    if src.nodata is not None:
        band = np.where(out_image[0] == src.nodata, np.nan, band)
...
ch_raster3 = np.where(ch_raster3 == 0, np.nan, ch_raster3)
fig, ax = plt.subplots(1, 1, figsize=(10, 3))
im = ax.imshow(ch_raster3, extent=(bounds[0], bounds[2], bounds[1], bounds[3]))
...
#| code-fold: true
#| fig.cap: "Various raster geometry types"
# stars raster cells, once converted with st_as_sf(), are just
# polygon geometries with an affine transform applied to their
# corners; the same effect is achieved here directly with shapely,
# without a Python equivalent of stars' `affine` dimension parameter
import geopandas as gpd
from shapely.geometry import box
from shapely.affinity import affine_transform
import matplotlib.pyplot as plt

def grid_cells(nx, ny, dx=1.0, dy=1.0, x0=0.0, y0=0.0):
    return gpd.GeoSeries([
        box(x0 + i * dx, y0 + j * dy, x0 + (i + 1) * dx, y0 + (j + 1) * dy)
        for j in range(ny) for i in range(nx)
    ])

regular = grid_cells(5, 4)
rotated = regular.apply(
    lambda g: affine_transform(g, [0.985, -0.174, 0.174, 0.985, 0, 0]))
sheared = regular.apply(
    lambda g: affine_transform(g, [1, 0.2, -0.3, 1, 0, 0]))

xbreaks, ybreaks = [1, 2, 3.5, 5, 6], [1, 1.5, 3, 3.5]
rectilinear = gpd.GeoSeries([
    box(xbreaks[i], ybreaks[j], xbreaks[i + 1], ybreaks[j + 1])
    for j in range(len(ybreaks) - 1) for i in range(len(xbreaks) - 1)
])

grd = gpd.GeoSeries(grid_cells(8, 5, dx=10, dy=10, x0=-130, y0=10),
                     crs="OGC:CRS84")
curvilinear = grd.to_crs("+proj=laea +lon_0=-70 +lat_0=35")

fig, axes = plt.subplots(2, 3, figsize=(10, 6))
for ax, geo, title in zip(
    axes.flat,
    [regular, rotated, sheared, rectilinear, curvilinear],
    ["regular", "rotated", "sheared", "rectilinear", "curvilinear"],
):
    geo.boundary.plot(ax=ax, color="black", linewidth=0.5)
    ax.set_title(title)
    ax.set_axis_off()
    ax.set_aspect("equal")
